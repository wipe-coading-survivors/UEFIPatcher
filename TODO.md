...
* [ ] **IfrBuilder: зеркалировать нативные флаги one_of (`0x10/0x10`)**
  ...
  и сохранение с 0-флагами работают, но F10 сохраняет молча, без
  перезагрузки — UX-расхождение с нативными вопросами (в первой
  сессии прочитано как «не сохраняется»). Применить при любой
  следующей прошивке с новыми вопросами. Контекст:
...
* [ ] **COM-переезд (2F8/IRQ3) клинит обе консоли на S2-стеке** —
  SerialDxe (S1-сборка) сидит на фиксированном 0x3F8; SIO-callback
  переносит UART вживую → записи SerialIo идут в пустоту с
  посимвольными таймаутами, а ConSplitter синхронно гоняет вывод во
  все консоли, включая зависший Terminal → локальный TUI тоже
  ползёт/виснет; лечение — сброс CMOS-джампером. Кандидаты: S4
  (resource-aware serial / донорский AMI SerialIo) либо guard в glue
  (не поднимать консоль при нестандартных ресурсах UART). Контекст:
  отчёт `docs/reports/2026-09-06-serial-s3-e31-verdict.md` §3.2.
