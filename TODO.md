...
Первая строка SOL при буте rd450x — про «запущен на баре металле»
(формулировка владельца; точный текст строки не зафиксирован — при
следующем AC-окне записать дословно). Практика: появление этой строки на
SOL = «BIOS дышит» после AC-cycle/прошивки — якорь начала POST до всяких
DXE/OptROM-строк. Попутно это наглядная причина, почему вендорские AMI
(класс B) в QEMU не грузятся: SEC/PEI тыкается в конкретное железо платы
раньше любой DXE-фазы (см. item «VM-тир предфлаш-валидации» ниже).
