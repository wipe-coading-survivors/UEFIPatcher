...
* [ ] **Setup-формы 226D2IL лежат в FREEFORM_SUBTYPE_GUID-секции —
  collect_forms их не видит** (находка разбора «как биос рисует Setup
  без форм», 2026-09-21) — HII на образе есть, но не там, где мы ищем:
  файл `899407D7-99FE-43D8-9A21-79EC328CAC21` (классический AMI Setup)
  несёт COMPRESSION **ctype=0** (обёртка без сжатия) → PE32 70КБ (драйвер
  без ресурсов) + **FREEFORM_SUBTYPE_GUID (0x18), GUID
  `97E409E6-4CC1-11D9-81F6-000000000000`, 382 483 байта** — с офсета 20
  (GUID 16 + u32 заголовок) блоб сходится точной цепочкой из 8 HII-пакетов,
  7 из них form-пакеты (type 0x02), до последнего байта. На C275 тот же
  файл разложен иначе (ctype=1 LZMA → PE32 165КБ, формы в .rsrc) — поэтому
  там Forms View работает. Сделать: парсить HII package list из 0x18-секций
  в collect_forms → Forms View / hii set-value заработают на asr1; вопрос
  строк (в этом блобе строковых пакетов не найдено — титулы брать из
  соседних HII-файлов) выяснится при реализации.
