...
* [ ] **uefi-engine: рекон-кит для копания FFS/DXE/PEI модулей** —
  ...
  - доточить search: режим GUID (парсинг строки → 16 байт LE),
  ...
  - **анализатор `analyze`** (ядро идеи): по узлу/секции —
    ...
    Справочник шаблонов пополняется по итогам реверс-сессий —
    каждый новый вскрытый паттерн становится детектором;
