...
* Вне скоупа: runtime-запись в живой NVRAM (setup_var-класс) — нужен
  UEFI-приложение-агент, не задача флеш-патчера. (Уточнение
  2026-09-15, 450x: runtime-снапшот стора ЕДЕТ В ОБРАЗЕ — запись
  IntelSetup@0x8021e1 во FV@0x800000; патч снапшота в образе = в
  скоупе флеш-патчера, см. v10 в саге 450x.)
...
* [x] **Разведcycle завершен** (детали: docs/reports/2026-09-20-asr1-226d2il-setup-recon.md).
  ...
  байт-в-байт): ~~SOL = data[1] (0x500089), Above 4G = data[1141]
  (0x5004FD)~~ — атрибуция была перевёрнута (не тот тумблер при снятии),
  исправлена живой сессией 2026-09-21 (см. след. секцию): **4G = data[1],
  SOL = data[1141]**. Значения 00/01. Дефолты
...
### Живая сессия 226D2IL: финальная карта переменных (2026-09-21)

* [x] **карта закрыта живыми пробами** (детали + хауту:
  docs/reports/2026-09-21-asr1-226d2il-live-varmap.md; инструменты
  hack/asr1-efivars-snap.sh + hack/efivars-diff.py; снапшоты всех 69
  efivars — refs/amibcp/asr1-map/s0-baseline…s6-final, gitignored).
  Above 4G Decoding = Setup data[1] (0x500089, IFR **1215:0x13E** —
  live-экран North Bridge; формы 1025/1035 — призраки с тем же prompt).
  Форма 1158: COM1 = data[1140] (0x10B), SOL = data[1141] (0x10D),
  EMS = data[1156] (0x10F). Setup Prompt Timeout — стандартная UEFI-var
  `Timeout` (u16 LE, сек), вне Setup-блоба. Self-verify SOL: тумблер
  глушит мост, efivar-откат вслепую по ssh + ребут возвращают POST в лог.
* [x] **тест-константы переведены на live-вопрос**: ASR1_4G_ITEM =
  1215:0x13E (3.50 = риг); для 3.30 отдельная ASR1_4G_ITEM_330 = 1025:5
  (0x18-IFR 3.30 — другой билд: его 1215:0x13E смотрит на off=841).
* [x] **Timeout — HII-вопрос** (поправка к карте): форма 1031 «Boot», qid 0x4
  (numeric u16, EFI-varstore `Timeout`); set-value печёт вложенную копию в
  StdDefaults @0x50057A (проверено: 01→05 ровно 1 байт). Юзер-сборка
  ~/225d2i-3_50-ds1.rom: дифф от стока = 4 байта (4G/SOL/EMS/Timeout), все
  совпадают с живым known-good.
* [x] **ЦИКЛ ЗАКРЫТ: прошито и активировано (2026-09-21, вечер)** —
  ~/225d2i-3_50-ds1.rom (4 байта) прошит flashrom 1.6 из toolbox t-adm
  (только BIOS-регион 0x500000-0x7FFFFF по layout, `-c "W25Q64JV-.Q"`
  `--noverify-all`; iomem=strict обойдён одноразовым кваргом в BLS — хост
  CoreOS остался нетронутым). Первый бут пересеял дефолты САМ (без F9,
  в отличие от rd450x): s7-flash == baseline по всем пяти значениям, живая
  StdDefaults перечитана. Детали: отчёт 2026-09-21-live-varmap, раздел
  «Прошивка и активация». Сценарий «севшая батарейка = всегда наши дефолты»
  обеспечен и подтверждён физическим тестом: полное снятие питания →
  `s8-powerpull` — Setup/Timeout в диффе от baseline отсутствуют, все пять
  значений на месте, POST в SOL с холодного старта.
* [ ] **TUI: двухстрочная подпись EMS-блока** (форма 1158, два подряд
  идущих text-опкода «Serial Port for Out-of-Band Management/» + «Windows
  EMS») не отрисовывается как заголовок блока (однострочные COM1/SOL
  видны); движок экспортирует оба текста — править рендер TUI.
* [ ] **TUI: зависание после set-value** (asr1, форма 1215/Above 4G,
  2026-09-21) — перезапуск сохраняет правки; нужен репро (какое действие,
  какая форма, лог) для разбора.
* [x] **TUI: команда `:nvar list/set`** — закрыто мини-циклом 2026-09-21:
  `:nvar list [PATH] [--var NAME]` (PATH — целевой режим: панель + goto по
  узлу стора, без PATH — сводка по всем сторам, `--var` ищет переменную по
  имени; запечённые копии за барьером — только с PATH) и
  `:nvar set NAME --offset OFF --value VAL [--guid GUID] [--width 1|2|4|8]`
  (семантика offset — внутри данных переменной, как в CLI; после set кэш
  NVAR-панель инвалидируется). Комплишен (подкоманды/флаги/пути/имена/
  guid/width), Enter в NVAR-панель — prefill `:nvar set`, хелп: EX-команды
  + секция NVRAM STORE. 12 новых тестов (unit + integration по моку).
  Исходная мотивация: живая сессия 2026-09-21 задействовала CLI nvar set
  для Timeout (позже выяснилось, что он HII-вопрос 1031:0x4 и печётся из
  Forms View).
* [x] **форма 1025 — служебная свалка AMI** (1017 вопросов, пустой
  заголовок, формсет 985EEE91), включает вопросы вне живых экранов
  («Wirless device», опечатка вендора). Источник «призрачных» дублей
  промптов; живым экранам не соответствует — при поиске вопросов
  ориентироваться на титулованные формы.
* [x] **«%s»-заголовки форм 1220-1225** — вендорский плейсхолдер в строковом
  пакете (живой браузер подставляет имя устройства динамически); не баг
  экстрактора, UEFITool/AMIBCP показывают так же.
...
* [x] **Resolution 2026-09-13 (движок, CLI):** qid подтверждён живым
  ...
  **v2 (контрольный образ Supermicro-450):** владелец принёс рабочую
  прошивку того же семейства (`refs/fw/超微450.rom`, «bili-bili»):
  её фабричный шаблон IntelSetup содержит `0x531–0x534 = 00 00 00 00`
  — вендор запёк все 4 экземпляра «IOU0 (IIO PCIe Port 2)» в
  x4x4x4x4 посевом (наш HNX: 0x531–0x538=8×ff, 0x539=00 — Port 1
  x4x4 под леновский рейзер). RC-модули идентичны по GUID
  (UncoreInitPeim/NVRAMPei/PlatformInfo/PlatformEarlyInit) —
  «работающая бифуркация» там = посев из шаблона, не UI. Live-факт
  владельца: Processor-настройки того же формсета из UI применяются
  ⇒ сейв-путь жив; фаворит-гипотеза — экземпляр IIO (0x531 vs
  0x532–0x534). v2-артефакт сеет все четыре по рецепту вендора:
  `refs/amibcp/450x-unlock-bif-4x4x4x4-all.bin`, sha256 `5a63c75f
  2ae0b444edd4d72d7284391c29789e9e7384152279aaae5737483834`
  (FV0-дифф ровно 4 байта 0x800745–48: ff→00; v1
  `450x-unlock-bif-port2-4x4x4x4.bin` с одним байтом устарел — не шить).
  **ВЕРДИКТ: PASS — 2026-09-13, железо (владелец, TMM-прошивка v2).**
  Port 2 разделился: `00:02.0-[03]` + `00:02.1-[04]`, под каждой по
  Optane; обе NVMe рейзера видны в ОС. Попутно применился фабричный
  `0x539=00`: Port 1 разделился на 1A/1B (появился `00:01.1`) —
  прямое доказательство посева. «Легенда о PEI-гарде Lenovo»
  закрыта окончательно. Побочка: TMM-прошивка сбросила NVRAM →
  runtime-настройки владельца слетели (console redirection:
  серийка молчит, POST-экран на VGA не рисуется) — восстановить
  руками в Setup (UI-сейвы доходят — Processor-настройки
  применяются) или испечь в v3 посевом Setup-стора. Открытый тред:
  контрольный билд 超微450 — владелец помнит UI-смену режимов
  бифуркации там; все 4 RC-модуля РАЗЛИЧАЮТСЯ (UncoreInitPeim
  606KB vs 628KB — другая сборка RC), сравнение потребления —
  кандидат на следующий цикл.
  **v4 (2026-09-13, исправление базы):** v2/v3 строились на pristine
  HNX99TF — а родная база владельца это `refs/amibcp/450x — копия.bin`
  (другой билд: 6.7M байт дельты, вкл. ME-регион; HNX — «тихий»
  hyperscale-билд, серийка/RC-дебаг молчат — отсюда побочка v2).
  v4 = родная база + REF3 + посев всех 4 экземпляров Port 2 (qid'ы
  копии: 0x243/0x258/0x26c/0x280 @0x531–0x534 → 0):
  `refs/amibcp/450x-native-unlock-bif-4x4x4x4-all.bin`, sha256
  `849876bfff74ba9bcbf78d05f0c08053a3c823d72690f89e05d1537409a2f569`
  (FV0-дифф 4 байта 0x8006fe–0x800701: 04 03 ff ff → 00×4).
  Форензика (уточнение владельца 2026-09-13): копия чистейшая,
  0x531=04 (x16) / 0x532=03 (x8x8) — фабрика под разводку платы
  (два слота x16 + четыре x8, один порт — DMI к чипсету), НЕ
  следы AMIBCP; феномен «AMIBCP правит — не применяется» закрыт
  чище: правки шли в SDP/IFR (по E13 — инертны для посева), стор
  оставался фабричным x16. v2 (на плате, работает, но без
  серийки и с даунгрейдом ME) и v3 (не финализирован) устарели —
  вернуться на v4.
  **v5 (2026-09-13, серийка+дебаг в посев):** свип промптов родной
  ...
  (или v5b с посевом).
  **Инвентаризация refs/fw (2026-09-13, владелец влил все копии):**
  450x-семейство = один BIOS-билд в пяти экземплярах: четыре
  идентичны («450x — копия»=450x-1=450x=450x-copy, sha 76ec3d3a…),
  450.rom и 450x-user-20230412.rom отличаются ТОЛЬКО ME-регионом
  (0xafb968-0xb292a1, 0xb492c0-0xc07fec) — update-пакеты, не
  live-дампы; NVRAM-гэп 0x840000-0x890000 пуст во ВСЕХ образах —
  рантайм-стор нигде не запечатлён (подтверждение модели:
  единственный доступный носитель — шаблон FV0+LZMA). Шаблоны
  450x-семейства идентичны: bif 04 03 ff ff 04…, dbg=01 (Minimum),
  trace=00, ConsoleRedirection=00 → рабочая серийка владельца жила
  в рантайм-сторе; v5 закрывает это посевом. X10DRH1_816.bin —
  настоящий Supermicro X10DRH (другая плата, разводка 03 03/03
  04) — только референс, НЕ шить на RD450x. IIO-pei32.bin (798880,
  PE32, строки «Invalid IOUx Bifurcation», «busUncore/busIio») —
  IIO-порт-конфигуратор из четвёртого билда, не совпадает с
  UncoreInitPeim наших трёх (606322/628434/627410); артефакт
  старых раскопок владельца, кандидат на дизасм. rd450xSPF.bin
  (297056, x64 PE, 5 секций, без строк) — по имени вырезанный
  $SPF/SetupData-носитель.
  **overclokers/ (2026-09-13, тред overclockers + китайские
  паки):** rd450-bf.rom ≡ 超微450 (побайтово, sha b3c406da…) —
  комьюнити-решение бифуркации = супер-микро билд с посевом
  0x531-0x534=00×4; полная валидация нашего пути (v5 делает то же
  на родном билде). Ещё 7 уникальных Lenovo-билдов (231 / 5.29 /
  602 / 219-bmc-nvme / 2rd450x / rd450x3 / rd450x4-ds1) — все с
  тем же шаблоном bif[04 03 ff ff 04…], dbg=01: v5-рецепт
  переносится на любой. RD450X~1.ZIP — скриншоты-инструкция
  «вход в BIOS → IntelRCSetup → pci-e 拆分 (бифуркация)».
  REBARU~1.zip — ReBarUEFI (ReBAR-мод; кандидат в роадмап).
  BMC.zip — BMC-прошивки гиперскейлеров (Baidu V3.24.1145,
  Tencent V2.17.1086 / V2.21.1095) + ipmitool. dxe-platform.ffs
  = FFS ABBCE13D (IntelRCSetup HII-драйвер, 89022 б, type 07) —
  вырезка для китайских модов. Архивы 打满睿频/不打满睿频鸡血 =
  turbo-unlock («鸡血») варианты; их шаблоны bif не тронуты.
  **v6 (2026-09-13, владелец прошил свой rd450x4-ds1):** серийка
  на ds1 ожила (CR в шаблоне ds1 = 00 → консоль идёт через
  BMC/SOL-путь, чьи настройки в TMM переживают прошивки; «тихий»
  HNX этот путь не слушался); дефолт IOU0=[x16] = родовой шаблон
  ds1; UI-флип IOU0 по-прежнему инертен (Port 2A Max Width x16 —
  модель подтверждена ещё раз). v5 перепроверена побайтово (посев
  лёг верно) — но не прошита. v6 = **ds1 владельца + ровно 4
  байта** посева Port 2 (store+0x8f9-0x8fc: 04 03 ff ff → 00×4;
  FV0-дифф 4 байта, всё остальное байт-в-байт его билд):
  `refs/amibcp/450x-ds1-bif-4x4x4x4-v6.bin`, sha256 `fd4610d95d
  553b11e643118f9dfa11eebc4210d32b755e90df0e4d25dd613237`.
  **v6-live (2026-09-14):** прошит; IOU0/IOU1 показывают [x4x4x4x4],
  НО IOU1-семью (Port 3) мы не сеяли (шаблон x16) → это вчерашний
  UI-сейв владельца → рантайм-стор пережил TMM-прошивку, ресиды не
  было; Port 2A остался Max Width x16 при значении x4x4x4x4 в
  рантайме → нативный билд игнорирует ручку из рантайма
  (подтверждено 2×). Template-путь на нативе НЕ проверен (ресиды
  не было) — тест: снос CMOS/NVRAM на v6 → первый бут → IOU0 из
  нашего шаблона? порт x4? **v7 собрана параллельно** = HNX
  (единственная база с доказанным посев→железо, v2) + REF3 +
  4×Port2=0 + «Redirection Support» #10005:0x85 @Setup+0x6 → 1
  (на HNX выключен по умолчанию — вместе с dbg=00 это объясняет
  «тишину» v2) + Serial Debug #9:0x1b → 3 Maximum + Trace #9:0x1c
  → 4: `refs/amibcp/450x-hnx-bif-debug-v7.bin`, sha256 `368677753
  33056f9036048b07108cb0384ec4e89a37bbfd2eb39902800c12220` (FV0:
  7 байт 0x80008e/0x8004ea/0x8004ed/0x800745-48).
  **v7-live ВЕРДИКТ (2026-09-14, владелец):** бифуркация PASS —
  Port 2 = 2A/2B, обе Optane + SM2263 в ОС; посев→политика→кремний
  подтверждён второй раз. Серийка МЁРТВА при всех трёх посевах
  (Redirection=1, Debug=Max, Trace=4) — и SOL, и физический порт:
  в HNX-билде код серийного вывода отсутствует (ручки в IFR есть,
  кода нет — вырезан гиперскейлером). Итоговая карта миров:
  HNX = бифуркация ✓ / дебаг ✗; натив (ds1) = дебаг ✓ (через
  BMC/SOL-путь) / бифуркация ✗ (политика игнорирует ручки);
  超微450 = бифуркация ✓ (комьюнити) / серийка молчала у владельца,
  но dbg=01 в шаблоне — кандидат v8 с посевом дебага.
  **v8 (2026-09-14):** разведка 超微450: корневой Setup имеет
  «Console Redirection» #10002:0x36 @Setup+0x4f и «EMS» 0x37
  @0x5f, COM-страницы 10053/10054; IntelRCSetup #9:0x1b/0x1c
  @IntelSetup+0x2da/0x2dd (размер стора 0x1677). Шаблон: ВСЁ
  выключено (CR=00, dbg=00, trace=00) — объясняет молчание билда
  в прошлом опыте владельца. v8 = 超微450 + CR=1 + Debug=3 +
  Trace=4 (бифуркация 00×4 уже в фабричном шаблоне, анлок не
  нужен — своё меню): `refs/amibcp/450x-sm-bif-debug-v8.bin`,
  sha256 `4786d9267c6ac3d64eb15f731b3da924a810de4eede5dffa84c2
  ccc3fad104ef` (дифф: 3 байта 0x430403/0x430406/0x431c9f +
  LZMA-репак). Тест: серийка ожила при bifurcation из коробки →
  «грааль»; молчит → последний путь «всё сразу» = дизасм
  нативного IIO-полиси (IIO-pei32).
* [x] **финал посев-пути на нативе: v5/v9 (2026-09-14, железо)**.
  v5 (натив+REF3+биф+консольные сиды Debug=Maximum/Trace=Enabled/
  Redirection=On): первый бут норм — TMM льёт весь 16MB включая
  NVRAM-регион, стор путешествует с образом, ресида на валидном
  сторе нет; F9 → наш сид + макс-дебаг → **POST-потоп в цикле**
  (0x8FFA4xA8-регистры), сетап недоступен, спас рефлэш.
  v9 = v5 минус консольные сиды (побайтно ≡ v4;
  `450x-native-v9-bif-seed.bin`, sha `849876bf…a2f569`):
  F9 → F10 → бут дошёл; IOU0=[x4x4x4x4] в сторе и на дисплее,
  Port 2A Max Width x16, lspci — один 02.0. **Вердикт: нативный
  IIO-консьюмер игнорирует IntelSetup@0x531; консольные ручки
  того же стора через тот же F9 применяются** — селективность
  консьюмеров, доставка работает. Модель: «зеркала BMC» нет,
  «выживания» нет (стор в образе: v6 везла база ds1); F9 =
  загрузка StdDefaults — наш сид и есть источник дефолтов.
  Опциональный остаточный тест: форс-ресид (v10 с затёртым
  NVRAM-регионом / сброс TMM).
* [x] **контрольная матрица ручек натива (2026-09-14, UI-пробы
  владельца + OS-дампы)**. Живые: ядра/HT, частота памяти
  (1600→1866), SOL-дебаг, **Link Speed cap Gen1 на Port 2A**,
  **PCI-E Port Auto→Disable**. Мёртвые: Override Max Link Width,
  бифуркация IOU0/IOU1/IOU2 (Port 2, Port 3 «второй x16», Port 1).
  Узор: мертва ТОЛЬКО «семья ширины звена» — скорость/enable/
  ядра/память/консоль из стора читаются. OS-дампы: I350 (01:00.0)
  LnkCap Width x4 Gen2 — **чип сам x4-распаян** (onboard-вариант),
  x4 на x8-порту без сплита; Optane (03:00.0) LnkCap Width x2 —
  рейзерный модуль x2-распаян, **разгадка вечного «Linked as x2»**
  (и после сплита он останется x2 в x4-слоте — скорость не
  вырастет, цель сплита = видимость обоих дисков).
  Коррекция (снята сверхтрактовка от того же дня): «Port 1
  фабрично сплит x4x4» НЕ доказан — суб-скрины 1A/1B существуют
  независимо от сплита, расхождения стор/железо по ширине не
  установлено; машина бифуркации на нативе не подтверждена (на
  HNX того же RC-семейства — работает). Гипотезы источника
  ширины для R1: data-таблица (UncoreInitPeim/board-blob),
  чтение страпа/GPIO (прогноз владельца: «обложен if false,
  то есть джампером»), константа, вырезанный if. Если источник —
  данные (таблица/страп-маска) → возможен чистый data-патч
  Port 2 (x16→x4x4) без единого чужого модуля.
* [ ] **engine: нестабильный RPC_INTERNAL у `hii question info`**
  (2026-09-14): на v5-образе первый вызов упал, после рестарта
  движка тот же вызов проходит; до падения в сессии открыто
  5 образов. Воспроизвести, найти источник (похоже на
  отравление состояния сессии первой ошибкой).
* [ ] **следующий цикл — свой бифуркационный драйвер вместо борьбы
  с чужими билдами (решение владельца, 2026-09-14)**. Контекст:
  натив игнорирует ручку — доказано v9 (политика захардкожена),
  HNX/超微 —
  чужие миры без SOL; отчёт-сага `docs/reports/
  2026-09-14-450x-bifurcation-serial-saga.md` §7. Физика
  (ответ на «когда запускаться»): конфиг портов — uncore-MMR,
  открываются ранним PEI; лей-маппинг защёлкивается НА РЕСЕТЕ
  (RC: программа → warm reset → тренинг); после тренинга заперто
  до следующего ресета. Варианты по возрастанию веса:
  (1) **DXE-лайт**: EDK2-драйвер (~200 строк) на Ready-To-Boot —
  читает IntelSetup 0x531+, пишет IIO-регистры напрямую, даёт
  warm reset; работает ЕСЛИ Grantley не лочит регистры после PEI
  (проверить!); инжект в DXE-FV — существующая механика add-FFS;
  (2) дизасм-патч нативного UncoreInitPeim (розетта — строки
  валидатора в `IIO-pei32.bin`); (3) кросс-свап UncoreInitPeim из
  HNX в нативный PEI-FV (одна RC-семья, риск HOB-версий);
  (4) полный свой PEIM — тяжелейший. Референс-карта:
  edk2-platforms **Purley** IIO (Skylake-SP — код той же фамилии,
  но НЕ Grantley: карта «что/в каком порядке», не бинарно).
  Шаг 1 цикла: дизасм нативного UncoreInitPeim (последовательность
  регистров/ресетов) + сверка с Purley-IIO → выбор варианта + EPA
  на записываемость IIO-регистров из DXE. Для PEI-вариантов
  движку нужна capability replace-FFS-in-PEI-FV. Старт — по команде
  владельца (brainstorm → спека → план).
* [ ] **v10 = патч runtime-записи IntelSetup в сторе образа (2026-09-15,
  ждёт прошивки)**. Разметка NVAR-стора FV@0x800000 (формат «NVAR»-
  записи: магия+uint16 размера+3 байта счётчика+attrs+guid-индекс+имя\0+
  тело): контейнер StdDefaults@0x800060 с вложенным дефолт-IntelSetup
  @0x8001b7 (тело с 0x8001cd — бывший «RAW-шаблон»), runtime-цепочка
  с живой записью **IntelSetup@0x8021e1** (g=7=EC87D643, тело с
  0x8021f7) + 12 стёртых призраков 0x167a. Развязки: дифф runtime-тело
  vs живой дамп = ровно 12 байт (все = ручки владельца) ⇒ «сеятель ≠
  шаблон» снят; v4/v9 сеяли ТОЛЬКО дефолт-контейнер, а PEI читает
  runtime-запись — механика провала v9; призраки исторически 03/04
  ⇒ bif-зона никогда не писалась через setup-save (guard на записи);
  TMM пишет стор образа (F9 после v9 показал наш 0 из зашитого
  контейнера). Артефакт: `refs/amibcp/450x-native-v10-var-bif0.bin`,
  база v9, дифф 1 байт 0x802728 (runtime+0x531): 04→00, sha256
  `57f960480c2ed440b882697f1b936f66ab8fa8353372f01e2a46bf7652ff6465`.
  Вердикт: `setpci -s 00:02.0 0x190.w` — 0008 = сага закрыта;
  0004 = guard НИЖЕ cfg+0x248 (цель дизасм-охоты сужается до
  ...
  **v11 вердикт (2026-09-15, прошит+бутнут):** регистр 0004; вход
  ...
  владельца: IFR не перечитывается каждый ребут — ручки жили через
  ребуты). В v11 уже нулевые все NVRAM-источники (runtime, FV0,
  FV2-LZMA) + 12 AUTO-констант. **v12 = перенос IFR-дефолт-флага**
  (0x30 с «x16» на «x4x4x4x4» вопроса #118:0x243, LZMA-рекомпресс
  слот-фит 0x14963+pad, sha `f821745e…c550ad4b`, engine-verify ✓) —
  закрыт пятый и последний известный источник 04; ждёт прошивки.
  Пляскин: TMM/AST2400 деградирует после 2-3 прошивок →
  `ipmitool mc reset warm` (наблюдение владельца).
  **v12 вердикт + ВЕРДИКТ ЦИКЛА (2026-09-15, 20:15):** v12 —
  регистр 0004, меню свежего бута [x16] (пост-флешовый пересеив
  читает не IFR-флаг). Ручной SOL-тест: IOU0=плюсом→[x4x4x4x4]→
  Save&Exit→ребут→**меню [x4x4x4x4]** (сейв пишет переменную, 00
  живёт через ребут) при регистре **0004** ⇒ guard ниже по потоку,
  доказано на железе; ретракция «write-guard bif-зоны» (04 в живом
  дампе v9-эры = порядок флешей). Финальная фаза: дизасм писателя
  0x190 в нативном PEI (xref регистра/width-последовательности),
  розетта = 超微-сборка (бифуркация работает на этом же железе;
  UncoreInitPeim 606KB vs 628KB). SOL-пляскин: F-клавиши >F4 не
  ...
  **РЕЗУЛЬТАТ v14 (2026-09-15 ночь, отчёт §J, коммит 6f2dd64):
  ЗАМОК ВЗЛОМАН.** Вся цепь исполнилась: «IIO Early Link Training
  Starting...» → «Socket[0] is socketValid=1» → «IIO=0, IOU0=0»
  (значение меню дошло до точки решения) → DumpIioPcieLinkStatus
  показал Port 2 = ДВЕ линии (D2:F0 x2 + D2:F1 x2 Gen3);
  0x190: 0004→**0000** (пишется RC; легенда «0008» не literal —
  кодировка иная). Дальше выяснилось: Optane сидит в SLOT3
  (x8-слот, port 2, LNKCAP самого Optane = x2 — полная своя
  скорость), SM2263 — port 3 x1 ВНЕ слотов (onboard M.2?, x1 —
  не регрессия). OMLW-оверрайды (2A=x4/3A=x4) ширин не меняют —
  лимит устройств. **Открытый вопрос владельцу (физика):** куда
  садить два Optane? Если SLOT3 (port 2) + SLOT4 (port 3) —
  бифуркация вообще не нужна (по порту на слот; проверить
  конфликт SLOT4 с M.2 на port 3). x4x4 на ОДНОМ порте нужно
  только если оба Optane в одном слоте/порте. Прошивочная часть
  саги закрыта; следующий шаг — решение владельца по рассадке.
  **SOL-клавиши (уточнение владельца, отчёт §K):** F-кнопки выше F4
  работают как **ESC+цифра** (F10=ESC+0, F7=ESC+7, F11=ESC+Shift+1);
  не работает только CSI `ESC[xx~`. Сохранение из любой формы =
  ESC+0. **Текущее ожидание:** тест райзера с двумя оптанами на HNX
  (дискриминатор: оба на одном порте = райзер бифуркационный →
  добиваем на 450x публикацию sub-портов 00:02.1+; разъехались по
  портам/не видны = райзер не тот или мёртв). Затем v15:
  пер-портовый «PCI-E Port»=Enable недоступен через меню (формы
  2B/2C/2D скрыты из GOTO-списка IIO0) — патч suppress/enum-пути.
  **v15 ГОТОВ (§K6, коммит d3bc38a):** `refs/amibcp/
  450x-native-v15-unhide-2bcd.bin` = v14 + form-unlock форм 87-89
  (suppress-ref чекбоксов присутствия 0xc51-53 выжжен ==0xFFFF),
  sha256 `5ac03eeca72d3c6ca4478032b190bebdf049b35894899faf2906657
  6286214cb`. ЖДЁT TMM. После прошивки: пересев → вернуть IOU0=
  x4x4x4x4 + Debug=Normal → в IIO0 появятся пункты Port 2B/2C/2D →
  на 2B «PCI-E Port»=Enable (var 0x735) → Save (ESC+0) → вердикт
  lspci 00:02.1+ / две NVMe на порту 2.
  **ТОЧКА КОМПАКТИФИКАЦИИ №2 (2026-09-15 ~23:10, всё ниже — в
  коммитах 4f521d2..1a09f8f этого дня):** сага на v15-ожидании.
  Состояние железа: 450x прошит v14 (в строю, ОС на нём), в
  переменной сохранены IOU0=x4x4x4x4, Debug=Normal, OMLW 2A=x4,
  3A=x4 (меню-сейвы персистентны). Хозяин бутует Хуанань (HNX) с
  райзером + двумя оптанами — дискриминатор бифуркационности
  райзера (оба на одном порте = да; разъехались = нет). Райзер
  пассивный, без переключателей. Реальная цель саги = умножение
  слотов: x16 Mellanox + x4x4x4x4 под 4 NVMe + x8 SAS-HBA + GPU.
  Дамп «вопрос→varstore-офсет» всего RC-формсета сохранён в
  `refs/amibcp/450x-rc-formset-question-offsets.txt` (4066 строк,
  регенерируется движком). SOL-риг и движок — сессионные, анкер
  выше описывает пересборку (fifo, F1-спам, ESC+цифра для F10,
  движок UEFIPATCHER_SOCK=/tmp/uefipatcher.sock). Логи бутов:
  /tmp/sol-v13.log (v13+Maximum), /tmp/sol-v14.log (v14-вердикт,
  меню-сессии, OMLW-эксперимент) — эфемерны, выжимки в отчёте §I-J-K.
  **ТОЧКА №3 (2026-09-16 ~00:50, отчёт §L):** v15 вшит и проверен
  живьём. Райзер-дискриминатор закрыт: рабочий HNX-райзер с 2×PM983A
  в ленове = один device (стандартный результат), на HNX оба оптана
  видны в BIOS сразу — райзер ок, душит ленова. На v15 + «PCI-E
  Port»=Enable на 2B (var 0x735) + OMLW 2B=x4 + warm/cold-ребуты:
  POST каждый бут печатает `Skt[0], D[2]:F[1] Link up as x04 Gen3!`
  (фаза «IIO Early Post Link Training»; наблюдение владельца «2
  устройства gen3 по x4» = вот это), но 00:02.1 в config space НЕТ
  (setpci → FFFF) — функция скрыта на уровне IIO. `00:05.0+0x80
  hdrtypectrl = 0x07` (clr_hdrmfd dev1/2/3 = «only F0 visible»).
  0x190 = сырой per-IOU селектор (2A=0000=x4x4x4x4 ✓), публикация от
  него не зависит; policy-хелпер F_2c76e пишет lane-маски в 0x45C
  каждого под-порта по функции — функции должны быть видимы ДО него.
  Безымянные чекбоксы присутствия живут в самих формах 87/88/89
  (последний вопрос после «Hide Port?», qid 0x3bf/0x3ed/0x41b ↔ var
  0xc51/0xc52/0xc53), навигационно недостижимы, OEM-дефолт 01 =
  «порта нет» — главный подозреваемый. **v16 ГОТОВ (§L3):**
  `refs/amibcp/450x-native-v16-presence0.bin` = v15 + set-value 0
  чекбоксов в оба StdDefaults-стора (store+0xdbe/dbf/dc0), sha256
  `4ad67b7bf9f30686c8ce19c5c86266f5d46e655af0865b36ab737483f14fd3
  fb`, NOP-ы v14 и флипы v15 на месте. ЖДЁТ TMM. После прошивки:
  F9 Load Optimized Defaults → заново IOU0=x4x4x4x4 + 2B Enable +
  OMLW x4 + Debug=Normal → Save (ESC+0) → вердикт lspci 00:02.1.
  Развилки при неудаче: (a) статик-охота на пишущего DEVHIDE
  (кросс-дифф nat/sm PE; определение регистра только в NDA EDS);
  (b) Broadwell-EP владельца (cfg+0x229ac→2 — OEM CPU-gen гейты
  проходят нативно). Бут-лог сессии: /tmp/sol-v15.log (эфемерен).
  **v16 ПРОШИТ И ПРОВЕРЕН (§L5, 2026-09-16 ~01:00):** чекбоксы
  отвергнуты чистым A/B (F9-дефолты 0 → 00:02.1 всё ещё FFFF,
  hdrtypectrl=0x07); live-хирургия `setpci 00:05.0 0x80.l=0` +
  rescan — 2.1 не появился (clr_hdrmfd = декорация, реальный гейт =
  NDA DEVHIDE в QPI-пространстве dev8, невидимом в lspci).
  **Находка: фабричный дефолт IOU0 = x4x4x4x4** (дефолтный бут
  пишет 2A.0x190=0000) — квад-сплит задуман платформой, OEM душил
  только трейнинг-замком (снят v14) + скрытием функций. Быстроскан
  PE на DEVHIDE-писца пуст (нет ни строк, ни 0x28080, ни конфиг-
  офсетных 0x80-сайтов) — нужен отдельный заход по всему образу.
  Машина на дефолтах, обе NVMe в OS. СЛЕДУЮЩИЙ ШАГ (владелец):
  **Broadwell-EP свап** — если pass скрытия проверяет cfg+0x229ac
  как гейт трейнинга, BDW открывает всё разом (v16 в прошивке
  безвреден); иначе — глубокий статик-RE. Лог: /tmp/sol-v16.log
  (эфемерен; выжимки в §L5).
  **v17 ГОТОВ (§L6, коммит d9e687a):** `refs/amibcp/
  450x-native-v17-baked-defaults.bin` = v16 + запечённые дефолты
  (Opt ROM: Launch PXE 02→00, NIC1/NIC2 ROM 01→00; Fast Boot/
  Fast Cold Boot → 0; Debug 01→02=Normal; 2B Enable 00→01;
  IOU0=0 уже фабрика), sha256 `ffda1c9339729b3d7c0a0b479017c9299
  a8a51b336084ea20e40e295d1baa6a3`. После прошивки — один F9,
  дальше SOL-сессии без ручного пересева. ЖДЁТ TMM (можно шить
  поверх v16 независимо от BDW-свапа — патчи кумулятивны).
  **v17 ПРОШИТ; BDW-СВАП ПРОИЗОШЁЛ (§M, коммит a46633a):** в
  железе теперь E5-2640 v4 (model 79); трейнинг сплита работает
  (D[2]:F[1] x04 Gen3), root port сменил DID на v4, но **скрытие
  2.1 НЕ снято** (FFFF, hdrtypectrl=0x07) — CPU-gen гипотеза
  отвергнута, hide = безусловный OEM-код. Гипотезы исчерпаны:
  menu-вары ✓, чекбоксы ✓, clr_hdrmfd ✓, CPU-gen ✓. ОСТАЛСЯ
  СТАТИК: (a) кросс-дифф nat/sm UncoreInitPeim всей площадью
  (sm без MZ — objdump -b binary; литерал-греп адресов мёртв,
  всё cfg-композируется; трассировать selector-copy 0x31f9b и
  per-port enable массив var+0x731+i → cfg → читатель);
  (b) перебор DXE RC-модулей; (c) ЕСЛИ донор-плата 超微 доступна
  владельцу — попросить `lspci -xxx -s 00:05.0` + `lspci | grep
  02:` с донора: сравнение живых регистров UBOX локализует
  реальный DEVHIDE-офсет → потом найти писца в nat по константе.
  BDW владельцу оставить (v4 = +4 ядра, саге эквивалентен).
  **ТОЧКА КОМПАКТИФИКАЦИИ №3 (2026-09-16 ~02:15, коммиты e7c8154..
  d1758d1):** v17 в железе + BDW E5-2640 v4. Canonical-пресет в
  NVRAM (вручную): IOU0=x4x4x4x4, 2B «PCI-E Port»=Enable, OMLW 2B=
  x4, Debug=Normal — вердикт неизменен (2B x04 Gen3 каждый бут,
  00:02.1 FFFF, hdrtypectrl=0x07). Механика пересевов (§M2):
  TMM-флеш/F9 → NVAR StdDefaults (наши печёные); смена железа →
  IFR-дефолты (OpROM Legacy, IOU0=x16) ⇒ **workflow: свап железа =
  F9**. F9 в v17-сессии не сработал (клавиши съедены) — проверять
  диалог по логу. Меню: попап (Enter на one_of) надёжнее '+'.
  Maximum-трейс (§M3): писец скрытия RC-дебагом не печатается,
  UniphyDisable=0 — путь один, статика. Артефакты охоты сохранены:
  `refs/amibcp/nat-uncore.pe` (798880b, .text VMA=0xffe64d28,
  file-off 0x220; imagebase-мапа PE↔VMA: +0xffe64b08),
  `refs/amibcp/sm-uncore.pe` (628224b, БЕЗ MZ — objdump -b binary),
  `refs/amibcp/450x-bdw-maximum-trace.txt` (405KB принтов).
  ОХОТА (план): (1) sm raw-дизассм; якоря-строки общие («IIO
  Early Link Training», «IIO=%d, IOU%d=%d.») → найти ссылающийся
  код → границы функций → байтовый сравнение nat/sm: одинаковые
  функции = общий RC-код, отличающиеся = OEM-кастом (кандидат на
  писца скрытия); (2) непрочитанные соседи в nat: F_918f8/F_95681/
  F_95f19 (обёртка «IioEarlyPreLinkTrainingPhase End» PE 0x2CD24,
  рядом с писателем 0x160); (3) если PE чист — DXE-модули RC по
  FFS-списку движка. SOL-риг: движок/BMC как в анкере №2; после
  F9-восстановления пресет печатается с первого POST.
  **ОХОТА ЗАВЕРШЕНА (§N, 2026-09-16): публикатор найден.** Цепочка:
  F_918f8 (11 портов; BDF из дефолт-таблицы PE VA 0xffe70e68 —
  все 11 подфункций вкл. (2,1)..(2,3); флаг cfg[0x2bc+sock*11+port])
  → F_9182d (ECAM + RP+0xA0/0x1B0, 16-bit RMW `and 0xffaf|0x40`,
  бит 0x40 = «опубликовать»). Живое: видимые 1A/2A/3A имеют
  0xa0.w=0x0c40 (бит 0x40 СТОИТ), 2B/2C/2D=FFFF; 0xA2[9:4] у 2A=4
  (x4). Флаги = префикс (0x2bc<0xc7d) policy-структуры 0x1670 из
  LocatePpi(2AB86EF5-ECB5-4134-B556-3854CA1FE1B4)→NVRAMPei (12/45,
  форвард к менеджеру); RC-дефолт в PE (VA 0xffe70ec0): +0x2bc все
  01 = публиковать все — прячется NVRAM-политикой (паттерн живых
  флагов в образе статикой НЕ лежит — вычисляется при сборке записи,
  потому меню/чекбоксы не помогали). F_918f8 НЕ гейтится на
  e0b/e37-пробах → **v18** = v17 + 11 байт @0xe918aa (F_9182d
  безусловный `mov $0x40,%ecx`+nops) = восстановление reference-
  семантики «публиковать все». `refs/amibcp/
  450x-native-v18-publish-all.bin`, sha256 764b899e6f223f729bb04e2
  ac251c583626f0997b0661e4c8e3c7369701fdf85. ЖДЁТ TMM. Вердикт:
  `lspci | grep 00:02` (ждём .0-.3) + NVMe на райзере; если 2.1
  есть, но multifunction-бит врёт — clr_hdrmfd с хоста (§L5).
  Воспроизводимость: `hack/uncore_publish_patch.py` (roundtrip
  проверен, sha256 сходится).
  Если НЕ появился → 0xA0.6 не гейт, след. кандидат DEVHIDE-писец
  в QPI-пространстве (dev8, NDA) — тогда nat/sm байтовый дифф.
  Движок/CLI-механика находок: node search bytes GUID, node
  extract+artifact export (GUIDed-секции разжимаются прозрачно),
  паттерн-поиск по образу python'ом.
  **§O (2026-09-16, вечер): v18 в железе — 2.1 НЕ появился; вскрыт
  самозамок presence → v19.** Пресет пережил TMM+F9 (0x190@2A=0000),
  но 2.1 из ECAM читает FFFF (живой /dev/mem-проб при
  `iomem=relaxed` — Bazzite без grubby, аргумент вписан в
  /boot/loader/entries/ostree-*.conf; bulk-чтения ECAM = FF, только
  поштучные dword; mmap /dev/mem требует fd r+b). dev8/9/10 (QPI)
  сами из ECAM невидимы; PEI-окна 0xB0000000/0xC0000000 (литералы
  0xb004xxx в nat-uncore: dev8+0x79/0x7c/0xb4/0x104/0x10c/0x18x/
  0x300) в рантайме мертвы. UBOX-карта по даташит-таблице §6.6
  расшифрована (mmcfg_base@0x90 и т.д.) — гейта там нет. Статика:
  оркестратор дочитан (F_9450d=XPUNCERRSTS, F_9196c=ERRCAP-нооп при
  нулевой пер-фн таблице 0xc7d, F_948c3/F_95f19=VT-d/стек).
  **Пробер F_96a0f расколот: VID-зонд → e37/e0b presence; скрытая
  функция = FFFF = «порта нет» → F_91ceb (конфигуратор RP+0x39c/
  0x3f0) скипает навсегда. У донора sm-uncore VID-зонда НЕТ ни в
  одной кодировке.** Писца DEVHIDE в PE нет (потребители policy
  перечислены исчерпывающе; в 23 карвах FV12 окон-литералов нет) →
  если не F_91ceb включает декод, то писец в DXE. **v19** = v18 +
  5 байт @0xe96a8a (`7405c60300eb04`→`9090c603019090`: je/jmp
  вырезаны, оба presence-байта безусловно 1), `hack/
  uncore_presence_force.py`, `refs/amibcp/
  450x-native-v19-presence-force.bin`, sha256 b9adce5038fe785fb658b
  2ca3883d1ff938180c54e94c26eb5603a5aac351e2, дельта 5 байт, lineage
  v18/v17/v15/v14 проверен. ЖДЁТ TMM; вердикт как у v18, развилки
  §O3 (DXE-декомпресс FV8 движком / ветки F_91ceb / SMM).
  **§O4 (2026-09-16, ночь): v19 тоже мимо (FF после TMM+ребут+F9 по
  SOL — вход в Setup = Ctrl+S по промпту, F9=ESC+9, F10=ESC+0);
  «вечный скип» F_91ceb = скип порта 0 (DMI), тело живо. DXE-охота:
  движок (чистый дата-дир, hand-made .uefipatcher по token из sqlite —
  session init RPC_INTERNAL), 207 модулей FV8 извлечены. НАЙДЕН
  IioInit (8/17, 63809859-…): функция @0xd2d0 — гейт
  `cfg[0x8cb+sock*11+port]==0 → скип` (je@0xd3cf), маска → UBOX+0x80
  (0x07-декорация) + dev4fn3+0xb0+fn*4 / dev7fn3+0x40+fn*4 (CBDMA/
  CTLE — dev6/7 = IIO MEM/PHY). Писатель 0x8cb=1 — сам IioInit @0xe1ef
  при e37≠0 + вызовы 0x121c8/0x1624c≠1; статический RC-дефолт +0x8cb
  все 00 (инверсия publish 0x2bc). Декод-гейт НЕ найден; v20-кандидаты
  §O4(a-e); патч в сжатой секции FV8 → write-режим движка (репак).**
  **ТОЧКА КОМПАКТИФИКАЦИИ №4 (2026-09-16 ~19:00, коммиты 5506fe6/
  fea58cd/f2f489f + этот):** в железе **v19** (TMM хозяина + мой ребут
  + F9 по SOL — дефолты подняты: 2A.0x190=0000=x4x4x4x4). Вердикт-
  цепочка: v18 (publish RP+0xA0) и v19 (presence-форс) — оба мимо,
  скрытие = декод-уровня (сырой ECAM 1.1/2.1/2.2/2.3/3.1 = FFFF;
  UBOX+0x80=0x07 — декорация, чистка с хоста не открывает; dev8/9/10
  и PEI-окна 0xB0000000/0xC0000000 из рантайма недостижимы). nat-uncore
  исчерпан (оркестратор F_91acc дочитан; «вечный скип» F_91ceb = скип
  порта 0/DMI; донор sm-uncore VID-зонда не содержит вовсе — но он
  другой RC-билд, пересадка модуля отклонена: раскладка cfg/версии
  протоколов не совпадают, донор = поведенческий эталон). DXE: найден
  **IioInit (FV8 8/17, GUID 63809859-F029-41C3-9F34-EEEB9EA787A5,
  x64, 100KB)** — гейт cfg[0x8cb+sock*11+port]==0 (je@0xd3cf, 6 байт),
  маска→UBOX+0x80, dev4fn3+0xb0+fn*4 / dev7fn3+0x40+fn*4 (CBDMA/CTLE);
  0x8cb=1 ставит сам IioInit @0xe1ef (гейт e37≠0 + вызовы 0x121c8/
  0x1624c≠1); статический RC-дефолт +0x8cb все 00; инвертированный
  потребитель @0xbecf (0x8cb≠0→скип, + лейны cfg+0xe63≠0 → вызовы
  0x13c04…). **Декод-гейт всё ещё не найден.** План v20: (a) NOP
  je@0xd3cf; (b) разобрать 0x121c8/0x1624c; (c) ветку 0xbecf;
  (d) полный аудит IioInit; (e) PciBus DXE. Патч в GUIDed-LZMA секции
  FV8 → нужен write-режим движка (репак как в NVAR-циклах v15-v17).
  Инфраструктура: движок на /tmp/uefipatcher.sock +
  UEFIPATCHER_DATA=/tmp/uefipatcher-data (эфемерно; `session init`
  тонет в RPC_INTERNAL ПОСЛЕ создания сессии — обход: вписать .uefipatcher
  руками, session_id+token из sqlite …/uefipatcher.db, таблица
  sessions); артефакты /tmp/dxe-scan (207 PE32 FV8), /tmp/tree19.txt,
  /tmp/iioinit.{pe,asm} — пересборка: node extract по путям из tree.
  Хост Bazzite (ostree, без grubby): **iomem=relaxed вписан в
  /boot/loader/entries/ostree-*.conf** (убрать по запросу хозяина);
  ECAM-пробы только поштучными dword (python mmap /dev/mem r+b — bulk
...
* [x] **список qid сценария B скорректирован** — план исключал 59 из
...
* [ ] **Терминальный рендер: цвет как на rd450x** — на rd450x (полный
  ...
  **Вердикт E33 (2026-09-07) — негатив, откат на E32**: serial
  глухая (единственная строка — RAW ERROR-статускод
  SOFTWARE/DXE_BS_DRIVER), Setup виснет на «Entering Setup...» на
  обеих консолях — вердикт-отчёт
  `docs/reports/2026-09-07-serial-s4a-e33-verdict.md` (разбор:
  (ii) опровергнуто статикой, гипотезы A/B/C). **S4b закрыт
  (вердикт E34, 2026-09-07)**: CMOS-clear + отсутствие glue —
  симптомы идентичны (Setup-вис, ERROR ×2) ⇒ D и B отвергнуты,
  TermSrc токсичен сам по себе в гибриде (A/C) — вердикт
  `docs/reports/2026-09-07-serial-s4b-e34-verdict.md`, резерв (а)
  закрыт (аддендум спеки §12). Хост откатывается на E32. Цвет —
  рамка «свойство TSE-вывода»: наш TerminalDxe уже эмитит ANSI-SGR
  без гейта (контр-факт выше), вопрос — шлёт ли LIVE-AMITSE цветные
  атрибуты в ConOut (на rd450x шлёт); пути: расследование
  AMITSE-рендера (отдельная спека) или путь (б). Парковка до
  решения владельца.
...
### Миноры усиления гейтов NP-P (ревью Task 5, 2026-09-09)

* [ ] **uefi-engine/real_image: whole-image byte-diff parity-ассерт** —
  гейты np1/np2 проверяют изменения на уровне файлов FV1 (порядок/
  GUID/тела, константный Δ), но не имеют явного ассерта «вне FV1
  образ байт-идентичен LIVE». Контекст: независимый python-дифф
  pack-отчёта E36/E37 это доказал (0 байт вне [0x890000,0xD60000)),
  но одноразовым скриптом вне репозитория; перенос в гейт закроет
  регрессию билдера (случайная правка вне FV1) навсегда. Несрочно:
  текущий билдер right-by-construction (пишет только дерево FV1).
* [ ] **uefi-engine/real_image: form-add «$SPF не тронут»
  байт-ассерт** — гейт np2 сравнивает $SPF до/после page add
  побайтово (список разрешённых полей), но для form add аналогичного
  прямого байт-ассерта нет (есть только count/длина/слоты в
  np_assert_ref_stage). Контекст: form add доказано не меняет $SPF
...
* [x] **Метка «NVRAM store» для RAW-файла AMI NVRAM** — закрыто: body-проб
  ...
  Сейчас отображается как «Raw CEF5B9A3-…» и выглядит загадочно (старый
...
* [x] **Setup-формы 226D2IL лежат в FREEFORM_SUBTYPE_GUID-секции — закрыто
  циклом 0x18 (2026-09-21)** (находка разбора «как биос рисует Setup без
  форм», 2026-09-21) — файл `899407D7-99FE-43D8-9A21-79EC328CAC21` несёт
  COMPRESSION ctype=0 → PE32 без ресурсов + FREEFORM_SUBTYPE_GUID (0x18),
  GUID `97E409E6-4CC1-11D9-81F6-000000000000`, 382 483 б, тело = AMI-список
  `[GUID 16][u32 = счётчик][пакеты без END до последнего байта]` (probe:
  1 строковый пакет на 3833 строки — титулы резолвятся из него же, слухи об
  отсутствии строк не подтвердились — + 7 форм-пакетов, 85 форм). Сделано:
  `parse_package_list_exact` (tolerant, exact-конец) + ветки 0x18 в
  `walk_sections`/`form_package_ranges` (каскад list_questions/set_value/
...
* [x] **Дуга «formset-unlock / перенос IIO-бифуркации» U1–U4**
  ...
  (запись выше).
* [x] **uefi-tui: Forms View скролл — курсор «едет» только вниз** —
  закрыт: персистентные ListState на App + `compute_scrolled_offset` в
  обе стороны (цикл tui-registry-polish, R9, 2026-09-18), затем
  forms/strings-списки унифицированы в `ui/scroll.rs` (цикл
  forms-panel-three-zone, 2026-09-19); тесты
  `forms_list_offset_kept_when_cursor_walks_up` и
  `strings_list_uses_persistent_state` зелёные.
* [ ] **uefi-tui: strings-браузер ('S') — обратный скролл подглюкивает на
  многострочных строках** — если текст HII-строки содержит переводы строк
  (\n/\r), `render_strings` кладёт его в один ListItem одним format!
  (`ui/forms.rs:164`) и элемент рендерится на несколько визуальных строк,
  а `strings_list_state`/`compute_scrolled_offset` (`ui/scroll.rs`) считают
  элементы — окно items рассинхронизируется с визуальным окном, при
  обратном ходе курсор/подсветка дёргаются (владелец, 2026-09-19, живой
  гейт после forms-panel-three-zone: «в Image View если выбрать строки 'S'
  — немного подглюкивает обратный скролл (из-за многострочных строк)»).
  Кандидаты фикса: (а) схлопывать переводы строк в один глиф (`⏎` или
  пробел) при рендере — одна правка в map; (б) однострочное превью +
  полный текст где-то отдельно. Контекст: `crates/uefi-tui/src/ui/
  forms.rs` (render_strings), `app.rs` (strings_list_state,
  strings_cursor), `ui/scroll.rs`.
...
* [ ] **uefi-engine: класс «suppress_if TRUE» не флипается (flip '-')** —
  ...
  и REF3→IntelRCSetup#116 в форме 4; вкладка Chipset отсутствует в
  живом баре именно из-за этого. Кандидат-фикс: побайтовая замена
  опкода TRUE→FALSE (length-preserving, 1 байт) как третий класс
  plan_flip (ConstantTrue). Разведка live-гейта 450x (2026-09-12,
...
* [ ] **uefi-engine: add_ref не валидирует существование цели REF** —
  ...
  REF target»; в живом Setup — мёртвый пункт меню). С `formset_guid`
  (REF3) цель тоже не проверяется — можно сослаться на несуществующие
  формсет/форму. Фикс: в `add_ref`/`check_ref_add` резолвить цель тем
  же способом, что и TUI (глобальная карта (formset_guid, form_id) по
  `collect_forms`; intra — пакет формсета-хозяина), нерезолв →
  InvalidSchema со списком доступных форм. Наблюдение владельца
...
* [x] **uefi-engine: extract/export и move форм/вопросов — лёгкая
  ...
  запрашивалось).
* [x] **uefi-engine: позиционная вставка REF/вопроса — IntelRCSetup
  ...
  Состояние (2026-09-22): цикл исполнен (PR #25, ветка
  positional-insert); артефакт `refs/amibcp/450x-positional-intelrcsetup.bin`
  (16 MiB, sha256 `e22bd0ddc2b04dac064a6fd1f7b4b4e84f1d8b3cf6791370a6c8ac
  91eeeb48c1`, схема `450x-intelrcsetup-tab.json` рядом) верифицирован
  инструментально — CLI- и TUI-пути дают байт-в-байт один файл,
  переживает рестарт движка, бар/ qid 0/ depth 0 подтверждены обходом;
  ждёт TMM-прошивки и SOL-вердикта. Факт: стоковые барные GOTO формы
  10000 несут qid 1..6 (легенда «qid-0 GOTO» из вердикта §7 снята,
  аддендумы в обеих спеках).
  Прошит 2026-09-22 (TMM): вкладка НЕ отрисовалась — гипотеза
  «qid 0 → вкладка» снята. Полевой дифф (аддендум «Живой гейт п.3»
  спеки positional-insert): наш REF3 нёс EDK2-маркеры vsid/QuestionId
  0xFFFF, родные REF3 формсета (рендерятся живьём) — vsid 0,
  QuestionId 0, последовательные ненулевые qid. v2 ГОТОВ (f931502,
  4c5d0e1): emit_ref3 → нативный AMI-паттерн, артефакт
  `450x-positional-intelrcsetup-v2.bin` sha256 cca796fb9780b42d7af4
  3f86d3b82fb4527282fe82a9e989d256523fdf958fca (qid 301, vsid 0,
  q2 0, якорь 10009 — вариант Б); ждёт TMM. Если не отрисуется —
  вердикт «топ-уровень бара недостижим через IFR на этом TSE».
  **v2 прошит 2026-09-22 (TMM + F9): вкладки НЕТ → вердикт по
  развилке: IFR-путь исчерпан (обе позиции × оба паттерна).
  Работаем план C: TSE root-page механизм.**
  План C, препарация TSE (2026-09-22, аддендум спеки): TSE =
  файл B1DA0ADF (PE 297КБ, «Aptio»); таблица страниц @0x1a80+
  блоками {формсет, form-id} — Server Mgmt = 01239999#2049
  @0x1c40, EC87D643#1 УЖЕ в списке (дважды); гейт видимости =
  NVRAM-переменная группы ({«ServerSetup»,01239999} @ф-я
  0x2eb0c); на риге IntelSetup-переменной нет (§O8) → вкладка
  не строится; F9 бесполезен (курица-яйцо). Развилки: C-тест
  (дамп NVRAM рига: ServerSetup есть/IntelSetup нет; asr1 —
  есть ли вкладка при живых EC87D643-переменных), C1 (RE
  драйвера ABBCE13D — кто создаёт IntelSetup; baked-дефолт в
  образе @0xe35eb0), C2 (TSE-патч мимо проверки переменной).
  C-тест ПРОЙДЕН (2026-09-22, живой дамп flashrom/t-adm,
  `450x-flash-live-20260922.bin`): риг байт-в-байт на v2
  (0x840000+); NVAR-магазин разобран — **IntelSetup ЕСТЬ и с
  контентом (5744б), ServerSetup флаговая (479б)** → обе
  гипотезы гейта (существование/контент) опровергнуты, легенда
  §O8 устарела. Реальный гейт — в коде построения root-page:
  фронт = секция-компаньон TSE (498КБ) / драйвер-кандидат
  ABBCE13D (PE @0xe35c30, несёт «IntelSetup»-шаблоны) +
  дизасм 0x2c3xx.
  Форензика Server Mgmt (2026-09-22, аддендум спеки): вкладка
  01239999#2049 — БЕЗ IFR-ребра (5 forms-пакетов, REF-ов на неё
  нет; класс 93039971 един у всех формсетов) → TSE добавляет
  программно; план C при провале v2 — таблица/хук root-page в
  TSE-модуле (кандидат B1DA0ADF). Легенда: формсет бара =
  7B59104A (899407D7 — GUID файла). База v2 чистая: trilock
  --check NOT PATCHED, дифф = 2 LZMA-региона.
  Фронт 1 ЗАВЕРШЁН (2026-09-22, аддендум «$SPF и настоящий
  гейт»): секция-компаньон B1DA0ADF = $SPF (компилированные
  страницы Setup, subtype FE612B72; 214 страниц, 161 из них
  EC87D643, форма 1 = page[37] с чистыми флагами; каталог
  переменных с размерами, совпадающими с живым NVRAM) — TSE
  рендерит бар НЕ из IFR (объясняет провал v1/v2). Настоящий
  гейт = hide-таблица @0x1b40 TSE PE: фаза-2 функции 0x2c108
  помечает bit0 страницы {EC87D643#1} и {7B59104A#10000}
  (контейнеры) — 01239999#2049 в ней нет → вкладка видна.
  **Артефакт v3 ГОТОВ к TMM**:
  `refs/amibcp/450x-intelrcsetup-tse-unhide-v3.bin` sha256
  9d5f6b553de1b7586f119bd7bebc83d5c95cfa44a5b003bc3da59ca7bc7b5b0e
  — занулена 32-байтная запись {EC87D643#1} @0x1b40 (одна
  переменная эксперимента; {7B59104A#10000} и терминатор не
  тронуты, единственный потребитель — 0x2c108). Дифф vs сток =
  одна область 0xafb9b0..0xb292a2 (LZMA-рестрим B1DA0ADF),
  trilock NOT PATCHED, тесты/clippy зелёные. F9 не нужен.
  Резерв при отказе: динамический список @0x1c00 (механика
  0x2eb0c), фронт 2 (ABBCE13D/PE @0xe35c30). Дампы: probes/
  tse-b1da0adf/{spf.bin,pe32.bin,tse.pe,tse.asm}.
  Прошивка flashrom из ОС НЕВОЗМОЖНА (2026-09-22, живой тест:
  BIOS_CNTL=0x2a, BLE + SMM-защита BIOS-региона; запись
  отвергнута, чип не тронут — проверено двойным чтением +
  /dev/mem; детали в аддендуме спеки). Риг безопасен, путь —
  TMM владельцем, F9 не нужен.
  **ФИНАЛ (2026-09-22): v3 прошит TMM — ВКЛАДКА IntelRCSetup В
  БАРЕ, БЕЗ F9. Кейс закрыт** (полная история и механизм —
  аддендумы спеки positional-insert: $SPF, гейт @0x1b40, живые
  гейты v1/v2/v3). Хвост: (а) операция «tse-unhide» в CLI/TUI —
  движок уже умеет, нужна обвязка+схема (см. мини-цикл ниже);
  (б) PR #25 смержен 2026-09-22 (вердикт владельца:
  «ДОЖАЛИ, КРАСАУЧЕГ»).



* [ ] **uefi-engine: мини-цикл «TSE unhide» — операция снятия
  hide-маркера root-страницы AMITSE** — артефакт v3 (живой гейт
  пройден 2026-09-22) собран temp-тестом поверх движкового API;
  нужна пользовательская операция. Значения для 450x получены
  ручной RE (дизасм + дампы probes/tse-b1da0adf/) — НЕ шаблон для
  произвольных BIOS, поэтому контракт полу-дженерик с сигнатурным
  поиском и guard-ами (не хардкод оффсетов):
  - TSE-модуль по GUID файла B1DA0ADF-4F77-4070-A88E-BFFE1C60529A
    (Aptio Setup Browser); проверить на соседних образах (asrock,
    mz32, 226D2IL3) универсальность GUID и секции $SPF (subtype
    FE612B72-203C-47B1-8560-A66D946EB371);
  - hide-таблица — сигнатурой: соседние блоки записей
    `{GUID(16), u64 form-id, нули}` stride 0x20 с нулевыми
    терминаторами (в 450x: @0x1a80 фаза-1, @0x1b40 hide,
    @0x1ba0 бар, @0x1c00 динамика); валидация: GUID-ы в записях =
    формсеты образа; guard: по найденному адресу лежит ровно
    {formset, form-id} из аргументов, иначе refuse;
  - правка = зануление 32 байт + Action::Rebuild по цепочке
    предков (обкатано: LZMA-рестрим guided-секции, FFS-чексуммы);
  - в комплекте $SPF-парсер (страницы/формсеты/каталог переменных;
    структура — аддендум «Фронт 1» спеки positional-insert) как
    рекон-команда для любых Aptio-образов.
  Делать отдельной сессией, по TDD (запрос владельца 2026-09-22).

* [ ] **uefi-engine: рекон-кит для копания FFS/DXE/PEI модулей** —
  уточнение владельца (2026-09-22): стартуем с чисто повторяющихся
  команд, блок функционала пусть вырастает; «углублённое
  ковыряние» = прежде всего АНАЛИЗАТОР бинарных секций на предмет
  известных шаблонов. Инвентаризация имеющегося — НЕ дублировать:
  `node search` уже ищет по name/utf8/utf16le/bytes (секции),
  `node extract` выгружает тело узла, `hii-form export` — формы.
  Дельта к существующему:
  - доточить search: режим GUID (строка → каноничные 16 байт
    EFI через uguid::to_bytes: data1/2/3 LE, data4 как есть),
    оффсеты вхождений ВНУТРИ тела узла (сейчас только сам узел),
    охват не только Section-узлов;
  - `guid-census`: цензус GUID-подобных 16-байтников с дедупом,
    счётчиками, позициями и аннотацией известных;
  - **анализатор `analyze`** (ядро идеи): по узлу/секции —
    справочник известных шаблонов и отчёт «что распознано и
    где»: магики ($SPF, NVAR, _FVH, MZ/PE+TE, LZMA-guid…),
    stride-таблицы `{GUID,u64,нулевые хвосты}`×0x20 с
    терминаторами (паттерн, вскрывший TSE-hide @0x1b40),
    `{GUID,имя}`-записи (ASCII/UCS-2), каталоги переменных.
    Справочник шаблонов — живой файл `docs/analyzer-patterns.md`
    (P1–P7 уже засеяны находками сессий 450x; правило пополнения —
    в AGENTS.md): каждый новый вскрытый паттерн становится
    детектором;
  - большой модуль НЕ проектировать заранее: команды добавляются
    по мере повторения паттерна в сессиях.
  Основа есть (parse_image + обход дерева + search/extract).
...
- Вердикт v21 (§O9, c189920): МИМО. F9 отработан (0x190=0 пережил),
  1B/2B/2C/2D/3B=FFFF, UBOX+0x80=0x07, NVMe живы, бут стабилен.
  Три замка (publish+presence+policy) сняты — декод закрыт всё равно:
  после-памятная цепочка nat-uncore ≠ декод. v21 = рабочий образ.
  След. фронт: (f) pre-mem MRC nat-uncore — писец DEVHIDE (вычисляемые
  адреса, литеральных сканов не было) + донор-дифф sm/nat по writer'ам
  ранней фазы; запасной план — SMBus/PECI-вид QPI-пространства.
- [2026-09-16 §P-анкер] v22 собран (заливается после TMM): вскрыта
  селекторная архитектура IOU — cfg+0x248/24c/250 (IOU0/1/2 = dev2/3/1),
  нат-дефолт IOU0=0 (донор 4); F_e90e48 пишет селектор raw в RP+0x190
  (бит3=start_bifurcation) ⇒ живой стенд: 2A.0x190=0 = x4x4x4x4 АКТИВЕН,
  трейс M3 «D[2]:F[1] Link up x04 Gen3» при ECAM FFFF; трансляторы
  F_e97ad7(PEI)/F_15d50(DXE) sel0→флаги «нет sibling-портов» — шизофрения.
  v22 = v21 + 5 байт @0xe97bb7 (sel0 ведёт себя как sel4 для IOU0).
  Инсайт: ECAM-записи в функции с выкл. декодом дропаются (master abort)
  — v18/v19/v21 не могли сработать механически. Обфусцированный писец
  F_ffef1e97 (0x03004040+4i / 0x0b004080+4i, константы $imm−reg+reg)
  = QPI-роутинг, НЕ DEVHIDE. При отрицательном вердикте: FV8-rebuild +
  F_15d50, фаза 0xe1ef, маршаллинг fff22xxx.
- [2026-09-16 §Q-анкер] v22 МИМО (2B/2C/2D FFFF, 0x190=0000 жив). Дедукция:
  PEI-читателей флагов нет ⇒ флаги не гейтят; DXE-патч отменён (master-abort).
  Timing-якорь: 2B читался живьём на моменте дампа линков ⇒ скрытие в окне
  [F_e91102 → DXE]; PipeExit-подфункции = PCU/хендшейк/ресет — не хайд.
  Резолвер SysAddress закрыт (F_e9f96b: comp/fn-таблицы → uncore BDF).
  Дальше: (1) живой трейс v22 Debug=Maximum + дифф к sol-max.log; (2) AMI
  FV12/DXE-модули на пер-портовые записи в 2B в окне видимости; (3) гейт
  0x229a4 у комп-0x01 RMW.
- [2026-09-17 §R-анкер] Фронт (1) ЗАКРЫТ отрицательно: Max+Trace-трейс
  (16MB, бут 37 мин — сериализация, не вис) — флаги v22 не изменили ни
  одного принта; окно [дамп→F1] пустое даже на Maximum; DXE на сериал
  молчит, IO-трейс умирает после BC; трейс-хук покрывает только MRC-CSR
  (bus-FF dev19/20) и AMI port-IO — RC-конфиг-путь (CF8/ECAM/resolver)
  не трейсится, QPI dev8-10 (DEVHIDE-пространство) не тронут ни разу.
  efivars: Setup/IntelSetup не экспонируются (сток = сырой NVAR, §O8.3).
  Сериал-инструментовка исчерпана. Настройки откачены (Normal+Disabled,
  бут 2 мин, инварианты целы). Дальше: (2) статический аудит AMI FV12
  (23 модуля) + publish-запись F_918f8 (0x40@RP+0xA0) до дампа; (3) гейт
  0x229a4; донор-дифф sm/nat; SMBus/PECI-вид QPI-пространства с хоста.
- [2026-09-17 §S-анкер] Фронт (2) закрыт с прорывом: publish-ветка
  дискредитирована кодом — F_9182d офсет-селект 0xA0+0x110·[F_97c98
  = видимый мост] ⇒ для 2B (видим в PEI, оба дампа x04 Gen3) запись
  шла в 0x1B0, мимо 0xA0; вызов F_9182d гейтится presence e37
  (F_e963e7), в стоке 2B вообще не публиковался. RP+0xA0 =
  линки-контроль (0x10 retrain / 0x20 link-enable F_e93d64/de5/e52),
  0x40 ставит тренинг-поток, не публикатор. QPI+0xFC = QPIPHYPWRCTRL
  force_l1 (выключение неиспользуемых QPI-линков 1S, принт RC), не
  DEVHIDE; после dump#2 записей в конфиг-пространстве нет вообще.
  Ункор-шины: QPI/R3/IRP = 3F (живой проб 3F:08.0 не делался — §O1
  пробил FF), IMC/PCU = FF. DXE-корпус (207 модулей) чист от
  литералов 2B; 8_61 = DXE-твин PipeExit comp-1 RMW (0x33333330,
  once-гейт), PEI-цикл не выполнялся ⇒ cfg[0x229a4] на 1S закрыт.
  Дальше (§S.6): (1) F_91ceb-декод-включатель (RP+0x39c/0x3f0,
  ветки F_96403/cfg+0x448); (2) живой проб 3F:08.0 + DEVHIDE-даунп
  (нужен хост); (3) DXE 8_61 идентифицировать + ревизия PciBus/
  платформенных DXE на runtime-BDF записи.
- [2026-09-17 §T-анкер] Живые пробы: шина 3F в ОС мертва (PEI-времянка),
  ункор = FF; QPILL0/1/2 (FF:08-0A) скрыты — +0xFC=1 из трейса = hide-self.
  RP-дампы: 3A+0xA0=0x0C00 БЕЗ 0x40 у видимого (0x40-теория добита),
  +0x1B0=0 у всех (v18-записи нигде не осели), +0xFC=0 у видимых RP.
  QPI-видимые (0B/0C/0D/1F) — масок 0x754/0x3A2 нет. Анхайд-записи
...
- [2026-09-18 §W-анкер] Прошивка BMC не удалась в ночь (хозяин: «шьем,
  но не перегружаем пока»): KCS-Yafuflash 3.32.1 (2013) сегфолтит на
  ядре 6.x ~20с после старта (успевая вкинуть BMC «Firmware Update
  Mode»; флаг дважды снят mc reset cold, BMC 2.36 цел); -nw не создаёт
  RMCP+-сессию; WebUI (80/443) и SSH-CLP-бэкенд в прошивке выключены.
  PDF E5 v2 #329188 (хозяин добыл): DEVHIDE-скрытые НЕ скрыты от
  JTAG/SMBus-порта IIO («turned off» скрыты всегда) ⇒ PECI-путь к 2B
  архитектурно гарантирован; офсет регистра и тут NDA. Состояние:
  IPMI жив, но PECI-движок после тудлов деградировал (sel1 ping был
  CC 00 → стал HAL -1; длины sel1 словно сменились — возможно
  dual-image reselect) → лечится AC-cycle, отложенным до окна.
  Дальше: (1) AC-окно → пинг до/после; (2) YAFU-реплеер на python
  (в Yafuflash 832 именованных функции — читаемо) → шить V3.24 →
  тест PECI; (3) если движок мёртв и там — V2.21.
- [2026-09-18 §W-доп: AC-окно] Хозяин сделал полный AC-cycle (ОС погашена
  gracefully по ssh, Chassis Power off по IPMI, тумблеры БП ~30с, обратное
  включение; BMC поднялся ~10с, хост поднят chassis power on, ОС за 80с,
  2A/6f04 на месте). ИТОГ: **PECI-пинг всё равно HAL -1** — движок не
  говорит с CPU даже с холодного старта ⇒ платформ-уровень (PECI не
  распаян/не стрэпнут на RD450x; сиптом-матч «отрицательная CPU-темпа»;
  CPU1 Temp, вероятно, с термодиода). Таблица длин sel1 после AC как была
  (3Б, 5Б=C7) — ранний 5Б-успех до update-mode тудлов не воспроизводится.
  Все три Yafuflash в пакетах идентичны (один билд 2015-04-08, тот же
  сегфолт). Остающиеся пути: (1) YAFU-реплеер → шить V3.24 (драйверная
  гипотеза, последний тест PECI-фронта); (2) при закрытии PECI —
  хост-BIOS-патч: DEVHIDE-регистр в видимом QPI-конфиге ⇒ модуль-дифф
  дампов ункора (конец PEI vs ОС) найдёт мапу хайда эмпирически, BIOS сам
  очистит биты 2B/2C/2D (флеш хост-BIOS у нас отлажен — v18-v22).

### SOL boot-якорь (2026-09-19)
Начало SOL-лога при буте rd450x (дословно, /tmp/sol.log, владелец):
`Checking for Reset Requests ...  Issue WARM RESET!` → `BIOS done set` →
`Checkpoint Code: Socket 0, 0xAF, 0x42, 0x0000` → **`Running on hardware`**.
Якорь «BIOS дышит» после AC-cycle/прошивки — строка `Running on hardware`
(ранний этап, до DXE/OptROM-строк; «запущен на баре металле»). Попутно:
`Checkpoint Code: Socket N, 0xXX, …` — готовые POST-прогресс-маркеры AMI
(0xAF здесь — практически конец POST). И это наглядная причина, почему
вендорские AMI (класс B) в QEMU не грузятся: ранний код тыкается в
конкретное железо платы раньше любой DXE-фазы (см. item «VM-тир
предфлаш-валидации» ниже).

## Инфраструктура: трекинг задач и VM-тир валидации (2026-09-19, владелец)

* [ ] **TODO.md → sqlite-трекер** — текстовый TODO разросся (~3.9k строк):
  статус открытости не виден глазами, нет дерева, хочется фильтры и
  JSON-экспорт (владелец: «уже полмесяца думаю»). Дизайн-повестка будущего
  цикла: (1) агентский workflow — AGENTS.md отправляет агентов писать в
  TODO.md, значит обязан быть CLI-контракт (`todo add/list/close/tree/
  export --json`), чтобы человек и агент работали одинаково, без ручного
  SQL; (2) git-диффабельность — бинарный sqlite не диффается: canonical =
  sqlite + генерируемый md/json-артефакт на коммит (или наоборот);
  (3) одношаговая миграция — разовый парсер текущего файла (секции `##` +
  items `[ ]/[x]` + вложенность) с верификацией «ничего не потеряли»;
  (4) место: отдельный маленький бинарь workspace или подкоманда uefi-cli.
  Контекст: `TODO.md`, `AGENTS.md` (раздел «Структура планирования»).
* [ ] **VM-тир предфлаш-валидации (QEMU/OVMF)** — отлаживать до прошивки не
  железо, а видимую часть: Setup UI (HII-формы после патча) + бут-уровень
  «есть/нет» (владелец, 2026-09-19: «перед тем как жечь»). Честный скоуп
  по классам образов: (A) EDK2-образы — OVMF 4MB ×2 и edk2-rk3588 уже в
  `refs/fw`: QEMU (pflash/`-bios`, screendump/VNC, serial-лог — серийная
  отладка уже обкатана циклами serial-s0/s1/s2) даёт реальный гейт
  «патч → бут → Setup виден» и заодно закрывает boot-уровень митигации из
  item'а про reloc-growth (выше в этом файле); (B) вендорские AMI-образы
  реальных плат (HNX99TF, 450x) в QEMU НЕ грузятся (инициализация
  конкретного железа) — VM для них НЕ гейт, остаётся текущий тир
  (parse/rebuild/диффы + живой риг rd450x по SOL). Кандидат первого шага:
  скрипт `hack/run-qemu.sh` (образ → QEMU → screendump + serial-лог) +
