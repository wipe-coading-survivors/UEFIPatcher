...
* [ ] **Терминальный рендер: цвет как на rd450x** — на rd450x (полный
  AMI-стек TermSrc, живой референс владельца) serial-вывод цветной; на
  нашем стеке (TerminalDxe) цвета/различия типов терминала не
  наблюдается. Косметика; механизм не разбирался (кандидаты-причины не
  утверждались).
