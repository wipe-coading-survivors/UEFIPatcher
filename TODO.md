...
## Аппаратная валидация unhide на Huananzhi X99-TF (2026-09-02)

> Живая прошивка программатором (та же плата, что в июльских
> экспериментах `~/git/BIOS`; их история — майнинг `/tmp/long-analyze.json`).
> Полный отчёт с побайтовыми уликами: `docs/reports/2026-09-02-hw-validation-hnx99tf.md`.
> Образы-эксперименты E1/E3/E5/E6/E7 и пробники — `refs/amibcp/` (вне репо).

### Критические находки (источник цикла фиксов)

* [ ] **ENGINE: LZMA-поток lzma-rs валит плату (видеописк)** — E1
...
* [ ] **ENGINE: пустые подписи раскрытой формы = нет PRC-токенов в
  x-UEFI-AMI string-пакете** — форма 901 «CPU HWPM State Control»
  открывается на железе (2 ONE_OF-вопроса рендерятся виджетами), но
  подписи пустые. Строки в en-US есть (4894/4965/4969), а в пакете
  `x-UEFI-AMI` (AMI-токены `PRC0003`/`PRC11B`/…) записей для них нет —
  вендор не создавал токи для скрытой формы. У видимых форм PRC-записи
  есть у всех отрендеренных строк. AMITSESetupData-записи вопросов
  (108-байтные, qid@0) для 0x0D5F/0x0D60 присутствуют — не причина.
  Нужно: (a) `string_pack` — вставка SIBT-строки на конкретный id
  (разрезание SKIP-пропусков, пакеты с дырами); (b) `set_item_visibility`
  — автоматический PRC-патч строк раскрытой формы. Июльский «unnamed
  rows» симптом — та же причина (строки добавлялись только в en-US).
* [ ] **hii string list отдаёт только первый string-пакет** — на HNX99TF
...
* [x] **R2 смягчён железом** — июль [07-19 11:00]: вставленная извне форма
  ...
  PRC, см. выше); E7: раскрытая форма работает. Блокер юзабельности —
  PRC-токены, а не принципиальный отказ AMITSE.

### Мелочи, найденные по ходу сессии

* [ ] **ops::rebuild не принимает GuidSection-таргеты** — `target_path`
  (ops.rs:130) возвращает NotFound для `<guid>:0x02:0`; rebuild работает
  только по Path. Временно обходится `node list` → путь.
* [ ] **artifact export пишется в CWD движка** — экспорт из CLI положил
  файлы в корень репо (движок был запущен оттуда). Резолвить путь
  относительно CWD клиента или требовать абсолютный.
* [ ] **Билдер: +2 байта хвоста декомпрессированного потока и +1 байт
