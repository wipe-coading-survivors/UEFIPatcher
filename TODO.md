...
* [x] **контрольная матрица ручек натива (2026-09-14, UI-пробы
  ...
  **PCI-E Port Auto→Disable**. Мёртвые: Override Max Link Width,
  ...
  Ключевое: **Port 1 (IOU2) физически уже сплит x4x4 с завода**
  (I350 Gen2 на 1A линкуется x4, 1B пустой «не трейнится») — при
  том что стор говорит IOU2=[x8]. Стор и железо расходятся по
  ширине УЖЕ на фабрике → политика ширины питается от
  захардкоженной таблицы платы, не от ручки; машина бифуркации
  на нативе РАБОТАЕТ (Port 1 — живое доказательство). Следствие
  для цикла A: главная цель R1 — найти таблицу портов (data-секция
  UncoreInitPeim или board-blob); возможен чистый **data-патч**
  (запись Port 2: x16→x4x4) — наименее «франкенштейновский»
  вариант из всех. Гвард-легенда: похоже, не гвард, а «ширина
  решается таблицей на самом раннем этапе, ручка просто никуда
  не подключена».
