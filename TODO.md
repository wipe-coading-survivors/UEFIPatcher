...
* [ ] **Терминальный рендер: цвет как на rd450x** — на rd450x (полный
  ...
  **Вердикт E33 (2026-09-07) — негатив, откат на E32**: serial
  ...
  (ii) опровергнуто статикой, гипотезы A/B/C). **S4b (2026-09-07,
  в работе)**: + гипотеза D (накопительная ConOut + TermSrc клеймит
  мёртвые терминальные пути) — кандидаты-дискриминаторы E34
  (без glue, после CMOS-clear) / E35 (glue v3 + TerminalDxe) готовы
  и отвалидированы (pack-отчёт
  `docs/reports/2026-09-07-serial-s4b-e34-e35-pack.md`), ждут
  прошивки владельца; дерево решений — в pack-отчёте §3. Цвет на
  E32-базе — через путь (б) или патч рендера, после S4b-дискриминации.
