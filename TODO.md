...
* [ ] **Дискриминатор A/B: применяется ли `PNP0501_0_NV[1]` физически**
  — (A) SIO переехал на 0x2F8 и донорный SerialIo последовал за
  ресурсами (resource-aware доказан контрастом E31/E32: там клин, тут
  жива) vs (B) ручка пишет вхолостую. Опциональные проверки владельцем:
  после Change Settings=2F8 → в ОС `dmesg | grep -i ttyS` (база
  ttyS0 0x2f8 = переехал); строка «Device Settings IO=…» в Setup;
  исчезновение раннего баннера при живой DXE-консоли. Не блокер.
* [ ] **Терминальный рендер: цвет как на rd450x** — на rd450x (полный
  ...
  не наблюдается. Владелец связал цвет с TermSrc (2026-09-06, «TermSrc
  тот же» — тот же модуль/GUID, билд платозависим: rd450x-билд ждёт
  Setup 0x94 с CR-полями +0x31..+0x3D, LIVE — 0x72 с иным лэйаутом;
  вставка = патч-карта спеки §8.3-а). Контр-факт: наш TerminalDxe
  `TerminalConOutSetAttribute` сам эмитит ANSI SGR-цвета (без гейта по
  типу) — т.е. отсутствие цвета может быть и не отсутствием TermSrc
  (кандидаты не разбиралась: доходит ли SetAttribute от AMITSE/Con
  Splitter до терминального чайлда). Дешёвая проверка без прошивки:
  Terminal Type = ANSI/VT-100+ и цвет рамок Setup в терминале;
  контроль — цвета локальной VGA-консоли. Путь лечения при
  подтверждении — резерв (а) полный AMI TermSrc.
* [ ] **E31-COM-клин не воспроизвёлся на E32** — контраст-факт (на E31
  ...
  жива); механизм E31-клина остаётся непонятым поведением (паркинг
  вердикта E31 §3.2). Снять с повестки при следующем аналогичном
  наблюдении.
