...
* [ ] **uefi-engine: рекон-кит для копания FFS/DXE/PEI модулей** —
  повторяющийся паттерн реверс-сессий (uncore-трилока, Server
  Mgmt, TSE/$SPF): найти модуль по GUID/строке (ASCII+UCS-2),
  цензус GUID-подобных 16-байтников с дедупом, детект
  сигнатурных структур (stride-таблицы {GUID,u64}/{GUID,имя},
  магики $SPF/NVAR/…), дамп поддерева/секции в файл под внешний
  дизасм. Кандидаты CLI-команд (накапливать по мере повторения
  паттернов, большой модуль НЕ проектировать заранее):
  `find-by-guid` / `find-by-string --enc ascii|utf16` /
  `guid-census` / `dump-node` / `extract-section`. Основа есть
  (parse_image + обход дерева). Запрос владельца 2026-09-22
  («нужен ли углублённый функционал по ковырянию модулей»).
