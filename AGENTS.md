...
- `docs/superpowers/journals/` — журналы реверс-сессий/экспериментов (`<дата>-<имя>.md`, например `2026-09-22-sm-ffs-transplant.md`). В `TODO.md` — только ссылка на журнал, чтобы TODO не пух.
- `docs/analyzer-patterns.md` — живой справочник шаблонов для анализатора (сигнатуры/структуры/таблицы; recon-kit). НЕ журнал. По окончании цикла/реверс-сессии, если вскрыты шаблоноподобные находки — предложи их пользователю списком, по строке на штуку (название + где найдено + сигнатура): «вроде нашли шаблоны, заносить?»; внести только подтверждённые, одной записью каждая (сигнатура+структура+где найдено+как детектить+первоисточник). Промежуточные находки и рассуждения — в спеку/отчёт цикла, не сюда.
- `TODO.md` — отложенные и несрочные правки (deprecated-код, известные дефекты, пробелы в API). Сюда же фиксируются находки ревизии CLI/кода, пока по ним не принят отдельный план. Дефекты движка помечать классом отказа в начале строки: `класс: silent-data` (молча неверные данные), `класс: write-path` (порча/полусостояние), `класс: errors` (путающие ошибки), `класс: limitation` (осознанное ограничение). Класс — поле радиуса поражения, не объёма работы; нужен для триажа и будущей миграции в sqlite-трекер.
...
9. **После каждой задачи** запускай `cargo test -p <crate>` и `cargo clippy -p <crate> -- -D warnings`. Финальный гейт цикла — дополнительно `cargo clippy --all --all-targets -- -D warnings` (тестовый код тоже).
...
### HII-код: чек-лист для новых walker'ов/каналов/мутаторов

> Вырастает из циклов hii-read-truth / hii-write-guard / hii-errors-cleanup (PR #26–#28). Нарушение каждого пункта там уже случалось — это грабли, не теория.

1. **Bounds — только через общий хелпер** `hii::ifr::package_bounds` (или существующий аналог модуля). Никаких `body.len()` в обход и приватных дублей хелпера.
2. **Арифметика id/offset/span — checked**, отказ или warn+stop. Никаких `wrapping_add`/`as u16` на string-id, count'ах и span'ах (прецеденты: `StringIdExhausted`, span-wrap u16, u32→u16 form_id).
3. **Целочисленные сужения — только try-конверсия** с диагностикой (`invalid_argument`), молчащее усечение запрещено.
4. **`unwrap_or_default` на каналах чтения запрещён**: пустой результат — осознанное решение (warn или `NotASetupItem`), а не дефолт по умолчанию.
5. **Усечённые/мусорные чтения — warn+stop**, не молчаливый 0/пустой список.
6. **Новый источник данных = read/write-сплит** селектора рангов (паттерн `form_package_ranges_read`): чтение видит шире, мутация — только разрешённое.
7. **Мутаторы**: порядок ошибок `InvalidItemId → NotFound → NotWritable`; no-op не вызывает flush; многошаговая мутация — plan-all-then-apply-all или snapshot-rollback (никаких частичных состояний на диске).
8. **Тест на обрезанный пакет — с точным ожиданием** (не `len() <= 1`).
...
cargo clippy --all -- -D warnings                # lint (межзадачный)
cargo clippy --all --all-targets -- -D warnings  # финальный гейт цикла (тесты тоже)
