...
- `docs/analyzer-patterns.md` — живой справочник шаблонов для анализатора (сигнатуры/структуры/таблицы; recon-kit). НЕ журнал: если по окончании цикла или реверс-сессии появился повторяемый шаблон (или похожее на шаблон) — оформи его ОДНОЙ записью (сигнатура+структура+где найдено+как детектить+первоисточник); промежуточные находки и рассуждения туда не сыпать — им место в спеке/отчёте цикла.
