...
## 16. Аддендум (2026-09-10, раунд 11): E39 — вопросы на сток-варсторе; контроль E32

Раунд 10 (вердикт §16): стена IntelRCSetup (уже на v7) + значения не
живут; корень несохранения — add_form с varstore не фиксапит $SPF
ifr-offset-ы (stage-track §16.2; TODO). E37v9 и вся v-серия с
varstore 31 закрыты негативом — не прошивать. Раунд 11 = обход бага:

| кандидат | файл | sha256 |
|---|---|---|
| **E39** (e16p-форма + страница + q600/q601 на varstore 1 @95/96 + REF) | `~/E39/E39-candidate-a49b8723.bin` | `a49b8723dd22b9384c13bac401d43cdbf4042db36c18f4e04c5daed50dba340f` |

Валидация (гейт np3, коммит `d62c86d`): $SPF 393 записи;
**инвариант round-11: 32 сток LIVE-записи остаются IFR-согласованными**
(каждая резолвится на своём финальном offset); q600@95/q601@96
резолвятся question_info; REF len 15/voff 0xFFFF/сток-END за REF;
вне FV1 — 0 байт; 187/216 сдвинуты (Δ=320); re-parse ок.

Протокол: **сначала контроль E32** (`~/E32/E32-candidate-8def2850.bin`,
sha `8def2850…`, розетка) — проверить IntelRCSetup (Advanced → выбрать
пункт): открывается? Это дискриминирует источник стены (наша дуга vs
платформа/NVRAM). **Затем E39** (розетка): страница «UEFIPatcher
Serial Settings» → два вопроса → переключить → F10 → ребут → живы ли
значения (теперь на сток-варсторе, прецедент E29/E31) → IntelRCSetup
открывается? Откат: E32 / база.

## 17. Аддендум (2026-09-10, раунд 12): E40 — бисект формы против REF

Раунд 11 вердикт (§17): E32 чист; E39 — значения живут (несохранение
закрыто), стена осталась. Раунд 12 = минимальный бисект дельты
E32→v7:

| кандидат | файл | sha256 |
|---|---|---|
| **E40** (= E32 + форма 10101 «UEFIPatcher» короткий title; БЕЗ REF/страницы/вопросов) | `~/E40/E40-candidate-03412ad9.bin` | `03412ad9b5f4bb7a1965aea445742b1ae9e8efc4c80a848e2f686f47c6c78f64` |

Валидация: рост in-slot (0 сток-файлов сдвинуто — чиже всех
кандидатов дуги), $SPF 391 (не тронут), REF в пакете отсутствует,
форма 10101 присутствует, re-parse ок.

Протокол (один флеш, розетка): IntelRCSetup (Advanced → пункт)
открывается? Серийный вывод жив? Наша страница в Setup недостижима
(нет REF) — ожидаемо. Интерпретация — вердикт §17.2. Откат: E32.

## 18. Аддендум (2026-09-10, раунд 13): E41 — форма в сток-диапазоне id

Раунд 12 вердикт (§18): стена — от ФОРМЫ (E40 без REF/страницы
стенит; структура пакета идеальна). Раунд 13 — дискриминация
form_id vs title-строки:

| кандидат | файл | sha256 |
|---|---|---|
| **E41** (= E32 + форма **10041**, короткий title, без REF/страницы) | `~/E41/E41-candidate-204ce5cc.bin` | `204ce5cc1602e52f78a3bac7cc4af3b7e7a444adae3bb79c207a3035ec7ac68f` |

Валидация: рост in-slot, 0 сдвигов, $SPF 391 (не тронут), форма
10041 присутствует, re-parse ок.

Протокол (один флеш, розетка): IntelRCSetup открывается? Серийный
вывод жив? Наша страница недостижима (нет REF) — ожидаемо.
Интерпретация — вердикт §18.2. Откат: E32.

## 19. Аддендум (2026-09-10, раунд 13-бис): E42 — форма + $SPF-страница

Раунд 13 (§19): E41 (форма 10041) стенит — форма-как-класс, id ни
при чём; дифф E32→E41 строго 8 Б IFR + строка. Дискриминация
«страница обязательна» vs «форма токсична»:

| кандидат | файл | sha256 |
|---|---|---|
| **E42** (= E32 + форма 10041 + $SPF-страница слот 188; без REF/вопросов) | `~/E41/E42-candidate-4810f11e.bin` | `4810f11e9e26b3d5f87ebcbea3ecc3cadc70704e5fcd1ea9e1fbceb3741fb329` |

Валидация: рост in-slot, $SPF count 189/слот 188/записи 391, форма
10041 присутствует, REF отсутствует, re-parse ок.

Протокол (один флеш, розетка): IntelRCSetup открывается? Прочие
пункты работают (ожидаемо да). Интерпретация — вердикт §19.2.
Откат: E32.

## 20. Аддендум (2026-09-10, раунд 14): E43 — пивот на сток-форму 10009

Раунд 13-бис (§20): E42 стенит → форма-как-класс (H2); парсер $SPF
в AMITSE не локализован → пивот вместо глубокой разведки: дуга
реанимирует мёртвую пустую сток-форму 10009, не добавляя ни форм,
ни страниц:

| кандидат | файл | sha256 |
|---|---|---|
| **E43** (= E32 + q600/q601 на форму 10009 (vs 1 @95/96) + REF q0x210 → 10009) | `~/E43/E43-candidate-7e5d4bba.bin` | `7e5d4bbaefa126e4cac30dbd56197f4bd6cea21dd66f566d3b79b4a1dda550bd` |

Валидация (гейт np4): REF len 15/voff 0xFFFF/target 10009/сток-END
за REF; форма 10009 несёт q600/q601; $SPF 393 записи ($SPF-страница
10009 обновлена клоном с контролами механикой add_question);
таблица страниц 188 не тронута; 32 записи IFR-согласованы; вне FV1
0 байт, re-parse ок.

Протокол (один флеш, розетка): (1) **IntelRCSetup открывается?**
(ключевая проверка — новых форм/страниц нет, ожидаемо ДА); (2)
Serial Port 1 Configuration → пункт «UEFIPatcher Setup» → вход на
страницу 10009 (заголовок стоковый; два вопроса Serial Console /
Verbose Boot); (3) переключить оба, F10, ребут — значения живы?;
(4) серийный вывод. Откат: E32 / база. Косметика заголовка —
отдельный шаг после приёмки.

## 21. Вердикт E43 (2026-09-10, владелец): ПРИЁМКА ПРОЙДЕНА

«E43: стены нет, наша форма есть, значения сохраняются» — все
критерии §20 закрыты: IntelRCSetup работает, goto-пункт ведёт на
страницу, вопросы переключаются, значения живут через ребут.
**Дуга setup-new-page закрыта положительно на E43**
(`7e5d4bba…` — эталонный образец дуги). E32–E42 закрыты
негативом/промежуточно — не прошивать. Возможные продолжения
(новые дуги, по решению владельца): косметика заголовка 10009
(E44: title-id $SPF-патч), наполнение страницы реальными
serial-настройками (замена демо-вопросов), фикс add_form×varstore
(TODO.md).
