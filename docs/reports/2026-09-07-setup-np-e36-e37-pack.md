...
## 15. Аддендум (2026-09-10, раунд 10): E37v9 — вопросы с $SPF-записями

Раунд 9: вопросы рендерятся, значения не живут — у q600/q601 нет
$SPF-записей (add_form пишет только IFR; вердикт §15). Раунд 10 =
та же цепочка, но вопросы добавлены `add_question`-ом (пишет IFR +
$SPF): форма 10101 пустая с varstore 31 (`np_form_v9.json`) + q600/
q601 (`np_questions.json`) на неё:

| кандидат | файл | sha256 |
|---|---|---|
| **E37v9** (вопросы + $SPF 393 + страница + REF) | `~/E37/E37v9-candidate-ba52655e.bin` | `ba52655ed94d869df3e1762a7d49ea9e0cbde253b68fb71eaa5478eb806b0a68` |

Валидация: $SPF 393 записи (q600/q601 на месте), REF len 15 / voff
0xFFFF / сток-END за REF, scope-balance = стоку, вне FV1 — 0 байт,
187/216 сдвинуты (Δ=112), re-parse ок.

Протокол (один флеш): розетка → E37v9 → приёмка: страница с двумя
вопросами → переключить оба → F10 → ребут → значения живы? Плюс
наблюдение: есть ли ERROR-строка на буте ДО входа в Setup (владелец
заметил 3000000/60000/A «и на предыдущем тесте» — на живом
кандидате; закрыть, меняется ли она с varstore-вопросами). Откат:
E37v8 / E37v7 / E32 / база.
