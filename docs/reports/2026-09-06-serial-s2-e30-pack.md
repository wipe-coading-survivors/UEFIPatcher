...
$ uefi-cli node insert 3/215 --file .../serial/SerialDxe.ffs --mode after
  3/215        # CLI печатает target (rpc/server.rs, обработчик node
               # insert); новый узел = 3/216
...
Запас: free_tail 2 082 028 → 1 958 888 Б (≈15,9× потребности).
...
- **PC-A — бут UEFI-путём.** Диск буст-приоритетом грузится по
  ...
  вывода не будет при рабочей вставке). Как убедиться до прошивки/
  бута: в boot-меню UEFI-записи имеют префикс «UEFI: » перед именем
  диска (легаси-записи — без него); рабочий диск с установленной ОС
  грузится UEFI-путём, если в boot-меню выбрана запись с «UEFI: » —
  E30-валидация должна использовать именно такую запись.
...
1. Перенести `/tmp/serial-s2/E30-candidate.bin` на машину прошивки
   (артефакт вне git — образы не коммитятся, спека §5). Кандидат
   лежит в `/tmp` — каталог не переживает перезагрузку хоста:
   скопировать в постоянное место до ребута (например `~/`) и
   сверить sha256 после копирования.
...
4. Ветка отката (обязательна): если после прошивки нет POST (экран
   чёрный, машина не стартует) — прошить обратно базовый образ
   E5C88C6F sha256
   `7e9d25443d5955e990f5f24cd1e7288d82e6655eb9c0f9f8c1ec5851de6a1c5d`
   тем же инструментом, что использовался в E26–E29. Вставка
   append-only поверх свободного хвоста FV1 — риск обрыва POST
   минимален, но ветка отката обязательна.
5. NVRAM-сброс/defaults после прошивки НЕ требуется (§4, PC-B).
6. До включения машины терминал уже должен быть открыт и подключен,
   UART-адаптер — на COM1 (маркеры идут рано: 115200 8N1 без flow
   control, готовность до подачи питания). Убедиться в UEFI-пути
   бута (PC-A), провести приёмку по §4. Результат (наблюдение из
   таблицы) — в вердикт E30.

## 6. Вердикт E30 и открытое

**Вердикт (2026-09-06, владелец): ПРОЙДЕН С ПРЕВЫШЕНИЕМ.** До прошивки
подтверждён вывод хост-ОС на ttyS0 (COM1) 115200 8N1. После прошивки
кандидата `09f5e897…` на COM1 рендерится Aptio Setup Utility целиком
(первая строка захвата: «Aptio Setup Utility - Copyright (C) 2019
American Megatrends, Inc.»; BIOS Information: HUANANZHI X99_15 x64,
Build 10/28/2019) — навигация стрелками работает, изменение и сохранение
настройки (таймаут загрузки) выполнены; после перезагрузки весь вывод
загрузки ОС идёт на serial. Гейт ступени — «UART-адаптер видит UEFI
boot-вывод» — перекрыт: работает весь маршрут (вставка+state-адаптация
→ DXE-диспетчеризация → SerialIo 16550 → TerminalDxe → ConOut-glue →
потребление AMITSE Setup-браузером).

Наблюдения по вердикту (не влияют на гейт):

1. **Маркеры SC-S1 не наблюдаемы.** Гипотезы (проверки не требуют —
   функциональность доказана рендером Setup): маркер 1 печатается в
   момент DXE-диспетчеризации glue, до старта AMITSE — его ClearScreen
   при отрисовке Setup затирает экран терминала (байты уже ушли в
   поток, но визуально стираются вместе со скроллбэком при
   переинициализации); маркер 2 печатается в ReadyToBoot через
   TextOut-child, сохранённый при glue-init — после ре-энумерации
   консолей BDS указатель вероятно stale, печать уходит в никуда.
   Диагностическая ценность маркеров исчерпана (E30 ответил на все их
   вопросы положительно) — пересборка glue не планируется; при будущей
   пересборке (S3+) — печать маркера 2 через gST->ConOut.
2. **0xB2-hang при входе в «настройки UEFI» из загрузчика NixOS**
   (перезагрузка в firmware settings): POST останавливается на B2.
   Штатный ребут из ОС и холодный старт — чистые (Setup доступен).
   Наше (третий консольный инстанс в энумерации TSE UI-пути) или
   платформенное — не установлено; A/B-тест на базовом образе = цикл
   прошивки, только по решению владельца (парковка TODO).
3. **Мусор 20–30 знаков при переинициализации железа** (псевдографика
   с «?»): давнее поведение платы (наблюдалось до дуги), с вставкой не
   связано.

Ступень S2 закрыта (аддендум спеки §7, roadmap); следующая — S3.

Наработка попутно (спека §4): real-image гейт `edit insert`
(`real_image_ops_insert_serial_s2`) + state-адаптация в `ops::insert`
— движковые, остаются в репозитории.
