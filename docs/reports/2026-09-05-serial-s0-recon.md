...
- Потребность S2: SerialIo 7 675 + TermSrc **13 361/13 403 по донору**
  (rd450x = 13 361, сумма **21 036**; MNX/Mashinist = 13 403, **21 078**
  — фикс R17) + выравнивание (прецедент фактической вставки: 25 294 Б
  на пару) — вилка потребности **21 036–21 078 Б** (форма §8.1).
...
- **SerialIo — один билд на троих**: FFS `472be948…` и PE32
  ...
  `Mashinist_DXE_driver_SerialIo` (4 источника). Исторические вставки
  волн 2–3 (E5C88C6F-эра, §1) использовали именно этот билд; для волны 1
  (v011: FFS 0x1E1C/0x44AE, §2.3) билд-атрибуция по размерам FFS не
  сходится — LZMA-перепаковка, улик нет.
...
4. **AMITSE-переменные: включать нечего и не где.** Ни один модуль
   ...
   (PNP0510_0_VV/NV, PNP0501_0_VV/NV, PNP0501_1_VV/NV), объявленные и в
...
Каждый пункт: решение → раздел-источник → чем доказано. Вход для
writing-plans S1 (конфиг пары) и S2 (целевой FV).

### 8.1 Целевой FV вставки S2

**Решение: FV1 (DXE) @0x890000**, name-GUID
`5C60F367-A505-419A-859E-2A4FF6CA6FE5`, вставка append в свободный хвост
после конца цепочки (LIVE: конец 0xB63B14 → первый слот 0xB63B18).
Источник §2.2/§2.4; доказано: (1) free_tail 2 082 028 Б, 100% 0xFF
(0xFF-скан + движковая сверка тома) ≈ 99× потребности S2 (вилка
21 036–21 078 Б: SerialIo 7 675 + TermSrc 13 361/13 403 по донору,
фикс R17; + выравнивание; прецедент фактической вставки 25 294 Б); (2)
том диспетчеризуем по
построению (DxeCore + DXE-apriori + 216 файлов; эмпирика — мусорный
COM-вывод v011+пара, §2.3-4); (3) все исторические попытки обеих эпох
вставляли ровно туда (§1.2, §2.3); (4) Setup-слоты 0x8D16D0/0xA9C708 в
середине цепочки — хвостовой append их не затрагивает. Риск-класс S2 —
не вместимость, а целостность пересборки существующих слотов (§2.4:
исторический дефицит — класс правок роста Setup-слотов, пост-4F §1.2);
append в хвост этим классом не является.

### 8.2 Конфиг edk2-пары (вход S1-плана)

**Модули: SerialDxe (9A5163E7) + TerminalDxe (9E863906) + glue-модуль.**
Источник §5.6.

**PCD** (§5.2, полный список — там): SerialDxe `PcdUartDefault*` — все
дефолты годны (115200/8/NoParity/1/1); SerialPortLib-слой
(BaseSerialPortLib16550) — все дефолты годны = COM1 0x3F8/115200/8N1
(`PcdSerialRegisterBase=0x3F8`, `PcdSerialLineControl=0x03`); TerminalDxe —
единственная правка **`PcdDefaultTerminalType=3` (VT-UTF8)** — дефолт
IFR «Terminal Type» обоих доноров (§3.4); `PcdErrorCodeSetVariable` —
дефолт.

**Glue: нужен.** В LIVE нет ни edk2-консольного стека (ConPlatform/
ConSplitter — 0/0, §5.4), ни терминального консьюмера SerialIo; пары
без glue молчали (§1.2, §5.4). Развилка реализации — за S1-планом
(§5.3-4): **(а)** мини-DXE `EfiBootManagerUpdateConsoleVariable(ConOut)`
до BDS; **(б)** прямое `gST->ConOut`-переназначение в ReadyToBoot
(минует переменную — переживёт любой BDS); **(в)** полный
ConPlatform+ConSplitter (тяжёлый, конфликтен с AMI-стеком). Отрицательный
контроль исторических вставок: gluer-а не было никогда (§5.3-4).
Альтернатива на случай краха edk2-пары — донорская AMI-пара, но только
через патчи §8.3-(а).

### 8.3 Уточнённые unknowns по ступеням

- **S4-путь** (§6.5): **(б) основной** — своя сборка (не зависит ни от
  Setup-лэйаута, ни от 560BF58A-семьи, ни от AmiSio; 0x3F8 напрямую);
  **(а) резерв** — бинарный патч донора: SerialIo патчей НЕ требует
  (бесконфигурационный, привязывается к детям GenericSio — SIO-семейство
  совпало, §6.4), TermSrc — imm32 ожидаемого размера Setup → **0x72**
  (фикс R23), серия imm8 CR-полей под лэйаут LIVE + 5 RIP-ссылок, +2–3
  NVRAM-записи (SerialPortsEnabledVar/SioSerialPortsLocationVar/
  DebuggerSerialPortsEnabledVar — в дефолтах §5.5 их нет); патч-карта
  индивидуальна per-билд (хрупкость класса (д)); **(в) гибрид**
  жизнеспособен только как «донорский SerialIo (без патчей) + наш
  TerminalDxe»; обратный наследует все патчи (а) и теряет смысл.
- **S3** (§3.6): геометрия «чужой» страницы переносима: обязательные
  поля (marker=1@+8, form_id@+0xA, title_id@+0xE == IFR FormTitle —
  инвариант 12/12 донорских + 23/23 LIVE, cnt@+0x1C, контролы 0x40-stride),
  72Б-раскладка записей неизменна на всех трёх платах; регистрация =
  zero-slot @0x64+4*count + bump count@0x60 (закрытые unknowns tse-bridge
  §3.5); плато-поля (u18@+0x18, B@+0x12, sig@+0x24, хвостовой слот) —
  копировать с образца LIVE. **Эталонный донор — MNX99MR9A** (полное
  семейство форм 0x2722/0x272C–0x272F, тот же билд формсета, что IFR_MASH,
  u18=0x3A как у LIVE); rd450x — контрастный образец проверки
  плато-независимости. Varstore: serial-семья уже декларирована в LIVE
  Setup-IFR (6 переменных 560BF58A) и фабрично включена (§5.5) — S3
  пишет в существующую семью, новых varstore не вводить (финальную
  схему «NV vs VV» — за планом S3).
- **S5** (§7.5): легаси-трое НЕ нужны — LEGACYREDIR (CSM-редирект
  легаси-вывода; DEPEX = полный LegacyBios-набор — без активного CSM не
  диспетчеризуется; конкурировал бы за порт; per-build Setup-привязка),
  SerialMiuxControl (IPMI/BMC-mux — DEPEX в LIVE неудовлетворим, 0
  вхождений IPMI), SerialRcovery (recovery-PEIM, один донор). AMITSE
  «включать нечего и не где»: TSE-канал serial = та же семья 560BF58A,
  уже фабрично включена (§7.5-4); AMITSESetup C811FA38 никто из
  легаси-слоя не читает. **Риск-фактор: CsmDxe** — при CSM-буте отбирает
  UART (DisconnectController по SerialIo-хендлам, §7.4): может фальшиво
  провалить E30-валидацию и конфликтует с любым UEFI-serial-драйвером.
  Активность CSM на HNX99TF S0 не установила — мини-pre-check S2+.

### 8.4 Статусы гипотез молчания (спека §2.3)

| | гипотеза | вердикт | источник |
|---|---|---|---|
| (а) | вставка вне диспетчеризуемого FV | **опровергнута**: 0xB23E08 = первый свободный слот FV1 (конец цепочки v011 0xB23E02 → align8), корневого DXE-тома; диспетчеризация эмпирически подтверждена мусорным COM-выводом v011+пара | §2.3 |
| (б) | донорские модули читают PCD-токены чужой PCD-базы | **опровергнута** (в сильной форме): PCD = только DEPEX-гейт SerialIo (удовлетворён PcdDxe в LIVE); PE-тела PCD-протокол не импортируют; конфиг пары = NVRAM-переменные (TermSrc) + SIO-протоколы (SerialIo) | §6.2 |
| (в) | SerialPortsEnabledVar и Setup-оффсеты не инициализированы | **подтверждена**: все три TermSrc-переменные — 0 вхождений в LIVE (→ NOT_FOUND → ветка дефолтов); каждый билд прибит к Setup-лэйауту своей платы (imm32 0x94/0x7D/0x7F vs LIVE **0x72**; imm8-оффсеты) — побайтовое доказательство: дифф MNX↔x99run = 19 платформенных констант | §6.2, §6.3, §6.4 |
| (г) | консоль не маршрутизирована в ConOut | **подтверждена для edk2-пары**: в LIVE нет терминального консьюмера SerialIo (SerialIo referenced только CsmDxe/AcpiPlatform/PciSerial; консольный стек целиком AMI Aptio); insert пары без glue молчал. Уточнение (§1.3): молчание — свойство E5C88C6F-эры; v011+пара дала мусор (порт работал); и (г) недостаточно для объяснения того молчания — пара добавляла консьюмера (TermSrc), остаточная причина — (в)/(д) | §5.4, §1.3 |
| (д) | «кастомные NVRAM-адреса / семья не инициализирована» | **частично**: сильная часть («семья не инициализирована на нашей плате») опровергнута — `PNP0501_0_NV = 01 00 00` фабрично в NVRAM-дефолтах CEF5B9A3 (+ зеркало 9221315B), семантика «COM0: Enabled»; часть «переменные/оффсеты, которых на нашей плате нет» — подтверждена (= (в)) | §5.5, §6.3 |

### 8.5 Вопросы, не закрытые S0 (→ мини-pre-check S2+ / планы S1/S3/S4)

1. **Активность CSM на HNX99TF** — определяет, сработает ли
   DisconnectController CsmDxe по нашим SerialIo-хендлам при CSM-буте
   (E30-валидация обязана исключить ложный негатив; §7.4/§7.5).
2. **AMITSE-BDS vs glue**: перепишет ли AMI BDS переменную `ConOut` /
   gST->ConOut после glue-варианта (а)/(б) — развилка §5.3-4, решение в
   S1-плане, эмпирика — E30.
3. **Карта оффсетов CR-полей LIVE Setup** (для S4-а): снять с IFR
   (§6.5); не снята — нужна только при выборе резервного пути (а).
4. **Точная модель SuperIO** (NCT5532D по коду драйверов vs NCT5567D по
   строке Setup, §6.4) — не блокер: семейство NCT55xx и AmiSio-продюсер
   GenericSio подтверждены.
5. **Терминальный тип владельца** для E30-приёмки (PCD=3 VT-UTF8 —
   дефолт доноров; какой эмулятор на стороне владельца — протокол
   валидации S2).
6. **Terminal 7A08CB98 (AMI, доноры 226)** — кандидат мини-pre-check
   S4: AMI-консюмер SerialIo без DEPEX, альтернатива TermSrc в гибриде
   (в); в S0 не распоряжён.
7. Методические: Tiano-compression слепое пятно (C275-PE32, §4.1) — не
   блокер (эталон rd450x/MNX); движок и EFI-COMPRESSION type 2 для
   226-подобных образов (R15); dword-immediate-проход сканеров и
   нерекурсия pe_scan по compression-секциям (R29/R31).

## 9. Артефакты разведки (воспроизводимость)

Скрипты в `/tmp/serial-s0/` (вне git), данные только чтение. Оговорка
хрупкости: `/tmp` не переживает перезагрузку — при утере артефактов
скрипты восстановимы из настоящего §9 (назначение, сигнатура вызова и
команда каждого) вместе с инлайн-сниппетами task-*-report.md;
единственный коммитимый скрипт дуги — `hack/fv_audit.py`. Сырьё (образы,
доноры, `refs/long-analyze.json`) — вне /tmp и не теряется.
