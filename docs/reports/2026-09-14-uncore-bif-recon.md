...
lane-map таблицы `0xffe755d8` (файл `0x10ad0`): для x16-IOU запись 0 =
`{04,04,04,04}` (x4x4x4x4), 4 = `{16,00,00,00}` (x16); для x8-IOU
(0xffe755d8+0xffe755dc... под-таблица 2×2): 0 = `{04,04}`. Идентичны в
HNX (`0xffde0aec`, файл `0x10bac`). Регистр: bits[2:0] = енум, bit3 —
код выставляет всегда (`or al, 8`).
...
0xE0000000 (Grantley-дефолт). Воркеры — сырой MMIO
(`0xffe9649b mov [esi], ax` write16; HNX `0xffdeeaef mov word ptr [eax], cx`).
...
подменяет AUTO→константу; первичный site — **файл-оффсет `0x3228A`**
(VA `0xffe96d92`, `mov byte ptr [edi + 0x248], 4`); дубл-пути:
`0x32317` (=4), `0x32492` (=3), `0x32633` (=3), `0x32665` (=2, райзер),
`0x326da` (=4). **Ручка IntelSetup+0x531 (IOU0) этим PEIM не читается
вовсе** — ни одной записи переменной/HOB в поля 0x248-0x251 нет
(исчерпывающе: прямые + lea-опосредованные записи перечислены, все —
константы {1,2,3,4}); сокет 0 никогда не получает 0 (x4x4x4x4) —
значение 0 встречается только для сокета 1 (`0xffe973c7`) и IOU1/IOU2
(`0xffe972ea`). Полный дифф-вердикт: полиси локализована, вход варианта 2
(патч imm 4→0 по `0x3228A`) валиден.
...
(дивергирующие ветки найдены, вход варианта 2 зафиксирован). Таблица MMR
...
(читающий пробник до write-фазы на MMIO-адресах вне таблицы).
