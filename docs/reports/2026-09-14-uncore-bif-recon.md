...
## P. Фронт (f): селекторная архитектура вскрыта, v22 = флаг-развязка IOU0 (2026-09-16, ночь²)

### P1. CSR-библиотека SysAddress расколота (конвейер pre-mem записей)

Хелперы nat-uncore (VMA): F_f066cc/F_f0676c (read), F_f06830/F_f068d3
(write; «SysAddress»), 435+141+722+215 вызовов. Адрес-аргумент =
(comp<<24)|(fn<<16)|offset; компоненты 3/8/0xb/0xc/0xd; спец-маршрутизация
компонентов через F_f6602 (нода = idx&1 / fn-биты), бэкенды F_e9fa03/
F_e9fbd1 → F_e9f96b возвращает MMIO-указатель (PCIEXBAR-окна pre-mem
0xb0000000/0xc0000000, формы 0xb004xxx = bus0 dev8; скип-лист хот-регов
в дебаге: 0x8024xxx/0xb0041xx). Донорский дифф иммедиатов: 365 общих
адресов, nat-онли 14 / sm-онли 6 — все версионые сдвиги офсетов
(0x37↔0x39, 0x77↔0x79, 0x84↔0x88), писателя DEVHIDE в литералах нет.

Карта зон по ASSERT-строкам (d:\aliang\rd450\t1\r450x231\GrantleySocketPkg\…):
IioEarly\{IioGen3,IioMmioAccess}.c+MemoryQpiInit.c = ffe921b4-ffe98d6f;
затем CpuPciAccess/PciAccess/InitMem (ffe9eaad+), Mem*, SysAddress-зона
fff066xx. Функции IioEarly пишут ECAM напрямую (F_e93f17/f30/f49/f62/
f76/f8a = read8/write8/read16/write16…, адрес = (bus<<20)|(dev<<15)|
(fn<<12)|off из cfg-таблиц 0xd1c/0xda3/0xda4): F_e92799 = назначение
bus numbers (0x18/0x19/0x1a), F_e92bc9 = SBR-секвенсер (RP+0x3e bit6:
set→2ms→clear→200ms; RP+0xA0 bit4 toggle), F_e92148-семейство читает
RP+0x190 и программирует лейн-мапы RP+0x3ac по таблице 0xffe75130.

### P2. Селекторы IOU: главный донор-дифф и «шизофрения» леново-сборки

* cfg+0x248/0x24c/0x250 = селекторы IOU0/IOU1/IOU2 = dev2/dev3/dev1
  (строки лога 'IIO=%d, IOU0=%d.' из F_e90e48). RC-дефолт политики =
  FF (в константе PE +0x248..+0x250 всё FF) → применяются кодовые
  дефолты инсталлятора ffe96cf3: **nat {IOU0=0, IOU1=4, IOU2=1} против
  донора sm {IOU0=4, IOU1=4, IOU2=1}** — единственное содержательное
  расхождение селекторных дефолтов. cfg+0x248 в nat НИГДЕ не пишется
  иначе как 0.
* F_e90e48 = PcieLinkTrainingInit: пишет селектор БЕЗ отображения в
  RP+0x190 c bit3 (iou_start_bifurcation, даташит §6.2.86
  pcie_iou_bif_ctrl: dev2/3 field 000=x4x4x4x4 … 100=x16; dev1 001=x8).
  Живой трейс M3 (sol-max.log): `IIO=0, IOU0=0 / IOU1=4 / IOU2=1` =>
  2A.0x190=0 = **x4x4x4x4 — расщепление dev2 в железе УЖЕ АКТИВНО**
  (1A=1=x8, 3A=4=x16 — прежние чтения 0x190 получают верную трактовку).
* Трейс M3 же: `D[2]:F[1] Link up as x04 Gen3` при живом ECAM FFFF —
  линки sibling-функций тренируются, декод выкл. ОТДЕЛЬНО (2C/2D Link
  Down = пустые ланки райзера).
* Транслятор селектора → пер-портовые флаги: PEI F_e97ad7 и DXE
  F_15d50 (IioInit, семантика 1:1). Ветка iou==0 (dev2, порты 4/5/6 =
  2B/2C/2D): sel1→{2B}, sel2→{2D}, sel3→{2B,2D}, sel4→{все три} в
  cfg[0x8cb+sock*11+port] и cfg[0x964+…]; sel0/INVAL → нули. Итого
  селектор ПЕРЕГРУЖЕН: в 0x190-пространстве 0=x4x4x4x4, во флаг-
  пространстве 0=«sibling-портов нет». Наш живой стенд = расщепление
  есть + софт не признаёт функции. Лейн-таблицы sel0={04,04,04,04}
  (F_e90d81, cfg+0xe66) согласны с 0x190 — противоречит только флагам.
* Инсайт master-abort: ECAM-записи в функции с выключенным декодом
  дропаются молча → ВСЕ после-mem форсы (v18 publish 0xA0, v19
  presence, v21 гейты F_91ceb) не могли механически сработать — их
  записи не дошли до цели. Гейт декода ставится раньше (pre-mem).
* DXE IioInit (8/17, /tmp/iioinit.asm): сам пере-устанавливает селекторные
  дефолты и ПЕРЕ-транслирует флаги (F_15d50, вызовы из инсталлера
  ~0x14c9d); поздний перекомпьютер @0xe1ef ставит 0x8cb/0x964=1 для
  портов, прошедших проверки (гейт cfg[0x448]≠2, cfg[0x112]≠1,
  предикаты 0x121c8 «multifn+SLTSTS-бит» и 0x1624c «class 068000»).
  Дефолты политики 0x112/0x264/0x448 = 00 → статические гейты проходят.

### P3. Обфусцированный писец найден (почему литерал-сканы молчали)

F_ffef1e97 (nat): 12 dword-записей через SysAddress-писатель F_f068d3
по адресам, собранным как `$imm − (esi−7) + esi` (константа не видна
в литералах!): **0x03004040+4i** (компонент 3 = broadcast) и
**0x0b004080+4i** (bus0 dev8 +0x80..0xAC — QPI-пространство), значение
= 6×2-битных полей (из ctx+0x810e+4i, per-socket stride 0x7902) +
счётчик<<12. Донор: те же константы −2 (0x3E/0x78) — версионый сдвиг.
Источник ctx+0x810e — CopyMem из стековых массивов функции fff22xxx
(маршаллинг per-port policy, stride 0x835×8 портов). Форма (2-битные
таргет-поля + счётчик) указывает на QPI-роутинг, не на DEVHIDE; сам
DEVHIDE-писец по-прежнему не локализован. Скрипт /tmp/obfscan.py
(идиома movl $imm,-N(%ebp); sub %reg,-N(%ebp)) нашёл только этот сайт.

### P4. v22 = v21 + флаг-развязка IOU0 (5 байт)

Селектор менять нельзя (0x190 + лейн-таблицы едут тем же байтом),
поэтому развязка в трансляторе: `hack/uncore_iou0_flags_patch.py`,
image 0xe97bb7 (VMA 0xffe97bb7 — дефолт-провал ветки iou==0):
`03 c6 88 50 04` (add %esi,%eax; mov %dl,0x4(%eax) — нули) →
`eb 05 90 90 90` (jmp на sel4-ветку 0xffe97bbe) — sel0 для IOU0
теперь даёт cfg[0x8cb/0x964 +4/+5/+6]=1, 0x190 и лейны не тронуты.
Артефакт **`refs/amibcp/450x-native-v22-iou0-flags.bin`**, sha256
`3e162393e8f38c6917cf4a2dd02bfb1c106a7fa12a5be9746502067d7c15e1ab`,
дельта от v21 = 5 байт @0xe97bb7; --check publish/presence/trilock/
iou0-flags все зелёные (линейдж v17+v18+v19+v20c-cave+trilock+flags).
Известный риск: DXE F_15d50 перетранслирует селектор и может сбросить
флаги в 0 — если вердикт отрицательный, следующий шаг: патч F_15d50
через rebuild сжатого FV8 движком (extract→decompress→patch→rebuild).

**Вердикт после TMM v22**: `lspci | grep 00:02` → ждём 00:02.0-3
(минимум 00:02.1); контроль инварианта: 2A.0x190 остаётся 0000
(расщепление не потеряно), 2A.0xA2[9:4]=4; NVMe на райзере.
Отрицательный вердикт ⇒ (a) FV8-rebuild + патч F_15d50; (b) фаза
перекомпьютера 0xe1ef в IioInit; (c) продолжение охоты за DEVHIDE-
писцом в fff22xxx-маршаллинге (источник 2-битных полей) и вокруг
PcieLinkTrainingInit.

## Q. v22 в железе — МИМО; окно скрытия сузилось до [DumpIioPcieLinkStatus → DXE] (2026-09-16, ночь³)

### Q1. Вердикт v22 — отрицательный, инварианты целы

Хозяин вшил v22 (TMM), F9 + ребут. Срез (ssh root-rd450x):
lspci = 00:01.0/00:02.0/00:03.0 по-прежнему; **сырой ECAM: 2B/2C/2D/1B/3B =
FFFFFFFF**; 2A.0x190=0000 (сплит жив), 1A=0001, 3A=0004; 2A.0xA2=0x3043
(появились биты 0x3000 — статус-биты слота); UBOX 0x80=07; PM983 (bus 03)
и SM2263 (bus 04) живы. Вывод: PEI-флаг-развязка декод не открыла.

### Q2. Дедукция после минуса

* В PEI нет НИ ОДНОГО читателя cfg[0x8cb]/[0x964] (только инсталлятор/
  транслятор пишут; массивы уходят HOB'ом в DXE) ⇒ флаги физически не
  могут гейтить pre-mem скрытие. v22 был обречён независимо от DXE.
* DXE-патч транслятора F_15d50 (через rebuild FV8) отменён как
  бессмысленный: после скрытия ECAM-записи в функции дропаются.
* Timing-якорь (трейс M3, перечитан): DumpIioPcieLinkStatus печатает
  «D[2]:F[1] Link up as x04 Gen3» — это РЕАЛЬНОЕ ECAM-чтение (F_e91102
  читает per-function статус-регистр 0xA2 через F_e93f76; при FFFF ширину
  0x3f трактует как «Link Down» и принт бы не вышел) ⇒ **конф-пространство
  2B было читаемо в момент дампа** — скрытие происходит ПОЗЖЕ.

### Q3. Аудит окна [F_e91102 → PEIM END]

Оркестратор (ffea7ed2): per-socket {F_ea7d5f, F_e90d52(лейн-таблицы),
F_e91acc(publish), F_e91b1c(F_91ceb→F_93ca8→F_90e48)}; затем F_ea7f3f:
print «IIO Early Post Link Training Completed» → per-socket F_e91b4b
(пост-тренировочные лейн-мапы: читает RP+0x3ac/0x3b0, зовёт
F_e92148/F_e9227c) → F_e91102 (дамп, 2B ещё жив) → «Continue with system
BIOS POST» → PipeExit (F_e998xx-функция): подвызовы F_ea7fe3/F_ea7cd2/
F_e99456/F_e9932d/F_ea5018 + RMW-цикл 0x01004040/44 (|=0x30, |=0x333300,
гейт cfg[0x229a4]==1, per-socket). Аудит подфункций:
* F_e99552 = PCU pcode-могла (0x13004040/0x13004044 |= 0x80000000,
  поллинг) — power management;
* F_ea7cd2 = хендшейк 0x8024094 ← 0x99 (поллинг до 0) + APIC ICR
  (0xfee00300 ← 0xc4500) — не порт-хайд;
* F_e9932d = reset-control (порт 0xcf9, F_e90c25/F_e90c3c/F_e90c46).
Прямого писца скрытия в PipeExit-окне RC НЕ НАЙДЕНО.

### Q4. Резолвер SysAddress закрыт (F_e9f96b)

SysAddress → (comp[31:24], fn[23:16]) → таблицы F_e9eb64/F_e9f410 →
(dev,fn) на uncore-шине; node-аргумент → номер шины (per-socket);
финал = ECAM_base(ctx+0x229f8) | bus<<20 | dev<<15 | fn<<12 | (addr&0xfff).
Компоненты 3/8/0xb/0xc/0xd/0x13/0x01 = логические юниты (QPI/Cbo/HA/PCU),
маппинг на BDF — таблицами. Все «странные» адреса писателей получают
однозначную трактовку.

### Q5. Следующие фронты

1. **Живой трейс v22 с Debug=Maximum** (SOL) + дифф против /tmp/sol-max.log:
   изменилось ли что-то с PEI-флагами=1 (реакция потребителей/HOB-дампы)
   и что происходит между «Continue with system BIOS POST» и Boot Agent.
2. **AMI-сторона окна**: 23 карвленных модуля FV12 + DXE — искать
   пер-портовые ECAM-записи в функции 2B (RP+0xA0-сброс, command-reg)
   в фазе, когда она ещё видима. RC-трейс хайда не печатает — писец
   либо AMI, либо «никто» (2B никто не скрывает, но и не публикует:
   бит 0x40@0xA0 ему никто не поставил — v18 ставил, но ПОСЛЕ скрытия
   и мимо; проверить время жизни бита: publish F_918f8 идёт в
   оркестраторе ДО F_e91b1c/дампа — пишет ли он реально в 2B?).
3. Гейт cfg[0x229a4]==1 у 4-сокетного RMW-цикла 0x01004040 — проверить
   живое значение (если цикл выполняется, комп-0x01 |= 0x30/0x333300
   требует идентификации).

## R. Фронт 1 (§Q5-1): живой трейс v22 Debug=Maximum+Trace — сериал-инструментовка исчерпана (2026-09-17, ночь)

### R.1. Постановка и ход

Setup через SOL (Advanced → Intel RC Setup → Miscellaneous
Configuration): Serial Debug Message Level Normal→**Maximum**,
Trace Messages Disabled→**Enabled** (попап-навигация; подсветка
пункта = bg black, детект через pyte-рендер экрана), Save&Exit.
Лог: `/tmp/sol-v22-max.log` 16.1MB (бут-1 Normal + Setup-сессия +
бут-2/3/4 Max+Trace). Бут с трейсом = **~37 минут** (каждый
MMIO/IO-доступ сериализуется в SOL; совпало с памятью владельца
«в прошлый раз 40 минут писало» — тогда рефлеш был перестраховкой,
ОС поднялась бы). Эпизоды: «B0-испуг» (поллинг-шторм 0x8FF980B4 у
Detect DIMM принят за вис — ошибочно: значения 0x8FFA5818 = живые
марджин-свипы, точного байтового периода нет); смерть SOL-моста на
буте-3 («session closed by BMC», ~1.5ч жизни сессии); пропуск F1 на
быстром буте → бут-4 для отката настроек.

### R.2. Вердикт по фронту 1 — отрицательный (инструментальный)

1. **Селекторы/флаги**: `IIO=0, IOU2=1 / IOU0=0 / IOU1=4` — идентично
   M3. Флаги v22 (0x8cb/0x964=1 для 2B/2C/2D) не изменили НИ ОДНОГО
   принта: новых строк нет, ассертов/ошибок нет (grep Assert/hide/
   publish/presence = 0). Печ-слой подтверждает §Q: потребителей
   флагов в PEI нет (статика + железо + теперь принты — тройное
   подтверждение).
2. **Линк-дамп идентичен M3**: D[1]:F[0] x04 Gen2, D[2]:F[0] x04 Gen3,
   **D[2]:F[1] x04 Gen3** (2B жив на линке в момент дампа),
   F[2]/F[3]/1B/3B/3C/3D Down, D[3]:F[0] x01 Gen3.
3. **Окно [дамп → F1-промпт] ПУСТО** даже на Maximum+Trace: никаких
   AMI PEI/DXE принтов (DXE: 0, Boot Agent: 0 — PXE выключен пресетом).
4. **Чекпоинт-таймлайн** (порт 0x80, 61 трейс-запись): A1→AF→B0/B1
   (MRC-проход 1) → **AF→A1** (перезапуск pre-mem — restore-fail после
   смены Setup → полный ретрейн; объясняет двойную пару «Detect DIMM
   Configuration») → B0→B6 (проход 2) → серия B7 ×17 (2.3MB–11.5MB)
   → B9/BA/BB/**BC** → линк-дамп → POST-экран/F1 → ОС. **После BC
   трейс-покрытие IO умирает** — DXE-фаза не видна вообще.
5. **Покрытие трейс-хука**: только MRC-CSR (bus-FF dev19/20 = IMC,
   ~300K доступов, штормы 0x8FFA0530/34 = марджин-свипы тетрадами
   0x03→0xA3→0xF3) и AMI port-IO (0x80/0x408). RC-путь конфиг-
   пространства (резолвер F_e9f96b) НЕ трейсится: CF8/CFC = 0,
   bus-0 ECAM-MMIO = 36 строк сантехники — UBOX+0x1C0 iiomiscctrl
   RMW 0x42050270→0x42050170 (снятие бита 8 tocmvalid, §6.6.51),
   dev0+0x1A0 RMW 3→0, PCH 0x800FB020/24.
6. **QPI-пространство dev8-10 (место DEVHIDE) в трейсе не тронуто ни
   разу** (гистограмма bus-FF: dev19/20 доминируют, dev11 = 17, dev8-10
   = 0) — писец скрытия либо идёт мимо трейсимого пути, либо исполняется
   вне покрытого окна.
7. **efivars**: Setup/IntelSetup не экспонируются (47 vars, из
   сетапных только 2-байтовый OemSaveCommonRC) — подтверждение §O8.3
   (сток = сырой NVAR-регион FV@0x800000); NVRAM-правок из ОС нет.

### R.3. Выводы и следующие фронты

Сериал-инструментовка (принты + IO-трейс) **исчерпана**: писец
скрытия в ней не наблюдаем в принципе. Остаются: (2) статический
аудит 23 AMI FV12-модулей + DXE на пер-портовые записи в окно
видимости 2B + проверка реальности publish-записи 0x40@RP+0xA0
(F_918f8) до дампа; (3) гейт cfg[0x229a4]; донор-дифф sm/nat;
SMBus/PECI-вид (даташит E5v2: скрытые устройства видны из SMBus/
PECI-порта IIO) — живой дамп QPI-пространства с хоста.

### R.4. Откат настроек: Serial Debug=Normal, Trace=Disabled (2026-09-17 02:34)

Сага отката: бут-3 прошёл мимо F1 (пропуск окна на быстром POST-хвосте),
бут-4 — смерть SOL-моста за минуту до промпта (третья за ночь: сессии
BMC живут под трейс-флудом ~10-13 минут, лог бод-рейт-лимитчен
≈310KB/мин → время линейно по офсету, F1 стабильно на ~мин 37.3,
шторм обрывается на чекпоинте BC). Бут-5 по расписанию: мост поднят
на мин ~33.7 → пережил 3.5 мин флуда → спам поймал F1 (02:32) →
навигация (попапы; попап открывается на текущем значении НЕ всегда —
подсветку проверять pyte-рендером bg black каждый шаг; в списке
Miscellaneous есть 4-й пункт RC Promote Warnings — счёт строк плывёт)
→ Normal + Disabled → Save&Exit. **Бут в ОС = 2 минуты** (против 37).
Инварианты после отката: 1B/2B/2C/2D/3B = FFFF, 2A.0x190=0000,
UBOX+0x80=07, PM983 (03:00.0) + SM2263 (04:00.0) живы — v22-статус-кво.

Откат завершён; rig на v22, Debug=Normal, Trace=Disabled. Грабли
ночи для протокола: (1) `kill $(pgrep -f "solrig.py …")` убивает
собственный shell (паттерн совпадает с cmdline) — только pid-файл;
(2) source-креды в фоновом скрипте — абсолютный путь и export,
иначе тихий KeyError; (3) 37-мин «вис» = сериализация трейса, не
клин — лечится только откатом Trace.

## S. Фронт 2: статический аудит + повторный мининг трейса — публикатор расколот, QPI-след закрыт (2026-09-17, вечер)

Хост не трогался (по требованию хозяина — живые пробы ниже помечены
«нужен хост»). Инструменты ночи: /tmp/ecamcensus.py, /tmp/timeline.py
(по /tmp/sol-v22-max.log), карвер /tmp/carve22.py → /tmp/carve22/
(62 PEI-модуля FV12, включая 047_UncoreInitPeim), DXE-корпус
/tmp/dxe-scan (207 модулей FV8, декомпресс прошлой сессии).

### S.1. ECAM-цензус трейса: карта полного конфиг-трафика

364 145 ECAM-доступов (1138 уник. адресов) за лог. Шины: **00**
(30 адресов), **3F** (343), **FF** (30 987 — спам марджин-свипов
IMC). Трейс-хук покрывает НЕ весь RC: доступы через F_e93f62/17/76/30
печатается (0x83F4312C виден инлайн в принтах QPI), но RP-путь
(IioPcieLib) — **нет**: в трейсе ноль bus-0 dev1/2/3 и ноль читов
+0xA2, хотя дамп читал 0xA2 у 2B живьём. Вывод §R.2 уточнён, а не
опровергнут: непрохукан именно RP-аксессор.

**Шина 3F = ункор-шина QPI/R3/IIO-стеков** (QPILL0/1 на 3F:08/09,
R3QPI на 3F:0C.0-7/0D, IRP на 3F:0B, PCU зеркало на 3F:1E);
IMC/PCU — на FF. §O1 пробил dev8-10 на шине FF — **не та шина**:
живой проб 3F:08.0 (ECAM 0x83F40000) никогда не делался (нужен хост).

### S.2. Таймлайн: мультPass-PEI, 2B жив на обоих дампах

Полный бут = **≥3 PEI-проходов**: после «QPI Init completed! Reset
Requested: 2» и «Pipe Init … Reset Requested: 2» идут ре-старты
(A1→AF→MRC→…). Дампы: #1 @строка 4567 (MRC прошёл, «IIO Early Post
Link Training», 2B x04 Gen3) и #2 @514113 (после ПОЛНОГО марджин-
обучения, 2B снова x04 Gen3). После dump#2 в трейсе нет НИ ОДНОЙ
записи в конфиг-пространство (кроме UBOX-декодов 00:05.0+0x0AC/0xD0/
0x10C-0x148/+0x1C0 и HB+0x1A0) до самой ОС. Итого в трейсируемой
вселенной писца скрытия не существует в принципе.

### S.3. QPI-след закрыт: +0xFC = QPIPHYPWRCTRL force_l1, не DEVHIDE

Принты RC называют регистры прямо:
- `3F:0x.3+0x12C` = **QPIREUT_PH_CTR** (RMW «Clr PhyInitBegin …
  write 0x4002000», строб |=1→2ms→&=~1) — код F_9371a (VMA
  ffe9371a; per-линк BDF из cfg[0xd1c]/0xda5/0xda6, линки из
  cfg[0xdaa]);
- `fn0+0xFC W=1` = **QPIPHYPWRCTRL «Set force_l1»** — принт:
  «Single Socket, no QPI Links to transition → Force unused links
  to disabled/low power state» — питание неиспользуемых QPI-линков
  1S-сокета. **Не DEVHIDE.** После этой пары (строка 5187) QPI-
  пространство не трогается вовсе → пишущего DEVHIDE в окне [дамп→ОС]
  нет. Механизм скрытия RP живёт не в QPI-конфиге.

### S.4. Публикатор расколит до конца: v18/v22 били не в тот регистр

Чистый дизассм (objdump -b binary, VMA−0xFFE00000=офсет образа;
нат-uncore.asm местами десинкрон от карва):

1. **F_918f8**(sock): 11-портовый цикл; гейт вызова — presence
   `cfg[0x2bc-арг не гейтит]`: зовёт F_9182d только если
   F_e963e7 = `cfg[0xe37+sock*11+port]≠0`; флаг публикации
   cfg[0x2bc+…] передаётся аргументом. Сток: 2B e37=0 (самозамок)
   → публикатор для 2B вообще не звался.
2. **F_9182d**(cfg, flag, packedBDF{b3=bus,b2=dev,b1=fn}):
   офсет-селект `0xA0 + 0x110·[F_97c98(fn,dev,bus)≠0]`, где
   F_97c98 = «девайс присутствует И REVCLASS==0x060000 (мост)».
   **Для видимого в PEI моста 2B запись уходит в 0x1B0 (NTB-
   раскладка), не в 0xA0.** RMW 16-бит: (val&~0x50)|flag?0x40.
3. Следствие: v18-патч (безусловный 0x40) в паре с v19 (presence=1)
   писал 0x40 в 2B+**0x1B0** — мимо. 0xA0-путь выполняется только
   для мёртвых портов (master abort, косметика). Двойной промах
   линейджа v18-v22 объяснён на уровне кода.
4. RP+0xA0 = линки-контроль, не «published-бит»: F_e93d64 — бит
   0x10 (retrain-строб, задержка 200ms), F_e93de5 — бит 0x20
   (link-enable), F_e93e52 — поллинг link-up (0xA2.0x800, до 10с).
   Живой 0x40@0xA0 у видимых портов ставит не публикатор, а
   тренинг-поток/железо. **Ветвь «publish-бит 0x40» как гейта
   видимости дискредитирована целиком.**

### S.5. DXE-корпус и гейт cfg[0x229a4]

207 модулей FV8: литералов ECAM/CF8 для 2B/2C/2D нет. Модуль
**8_61** (внутренний файл 61 DXE-FV, 572KB, стрипнут, PE32+) —
DXE-близнец PipeExit-цикла: `mov r8d,0x01004040/44` + аксессор,
OR-маски 0x33333330/0x333333 (те же, что §Q3), once-гейт глобальным
байтом. PEI-цикл в трейсе не выполнялся (нуль доступов к
0x01004040) ⇒ гейт cfg[0x229a4] на 1S закрыт — комп-1 RMW =
power/роутинг (не видимость), фронт §Q5-3 закрыт как неприоритетный.

### S.6. Следующие фронты

1. **F_91ceb как декод-включатель** (§O2-развилка b): дизассм
   RP+0x39c/0x3f0 RMW (F_93875-семейство: стробы 0x800/0x100),
   ветки F_96403/cfg[0x448] — мог ли включатель декода не сработать
   для 2B (преследование «декод по умолчанию выключен»).
2. **Живой проб 3F:08.0** (ECAM 0x83F40000, /dev/mem, только
   чтение): шина 3F в ОС не пробалась; если QPILL0 читается —
   полный дамп 4KB + поиск DEVHIDE-битов per-port (даташит: DEVHIDE
   именно в QPI-пространстве) и, при удаче, запись. Нужен хост
   (хозяин координирует).
3. Идентификация DXE 8_61 (протоколы/GUID) + ревизия DXE
   PciBus/AMI-платформенных модулей на runtime-BDF записи в RP
   (литеральный греп их не видит).

### T. Живые пробы вечера (хозяин: «бомби /dev/mem», только чтение + 2 пробные записи) (2026-09-17 поздно)

Метод: ssh root-rd450x, python mmap /dev/mem r+b, поштучные
выровненные dword (§O1-конвенция). Инварианты до/после зелёные
(2A=6f04; 03:00.0=8086:2522; 04:00.0=15b3:0212).

NB (уточнение хозяина, после проб): раскладка слотов сменилась ДО
этих проб — на ноге 2A (03:00.0) теперь **Optane** 8086:2522 (не
PM983), на 3A (04:00.0) — **Mellanox CX6 Dx Flash Recovery**
15b3:0212 (в ранних сводках его ID ошибочно подписан «SM2263»),
бут-флеш SM2263EN (126f:2263) уехала на чипсет-порт 08:00.0.
На выводы не влияет: охота — за декодом функций 00:02.1-3, а не
за эндпоинтами; все дампы §T сняты уже при новой раскладке.
Побочное: селектор 3A.0x190 теперь = 2 (на v17-эре §N был 4;
1A=1, 2A=0 не изменились) — RC пересобрал раскладку IOU2 под новое
население слота.

1. **Шина 3F в ОС мертва** (все dev fn0 = FF) — это PEI-времянка.
   Ункор в ОС живёт на **FF** (двойное отображение в PEI подтверждено:
   PCU читался и на 3F:1E, и на FF:1E).
2. **FF-цензус**: QPILL0/1/2 (FF:08-0A) = **FF, скрыты** (эффект
   +0xFC=1 из трейса = hide-self, а не только force_l1). Живы:
   0B.0-3 (6f81/6f36/6f37/6f76, fn4-7 скрыты), 0C.0-7 + 0D.0-1
   (R3QPI 6fe0-6fe9), 0F (6ff8), 10 (6f1d), 12 (6fa0), 13/14/15
   (6fa8/6fb0/6fb4), 16 (6f68), 17 (6fd0), 1E (6f98 PCU), 1F (6f88).
3. **Полные дампы RP 1A/2A/3A** (4KB, nonzero 63/68/66):
   - **3A+0xA0 = 0x0C00 — бит 0x40 ОТСУТСТВУЕТ у видимого порта**
     (теория «0x40 = published» добита живьём; 1A/2A = 0x0C40).
   - +0x1B0 = 0 у всех трёх — записи v18 (0x40→0x1B0) не видны
     нигде: публикатор на финальном PEI-проходе не отработал.
   - +0xFC = 0 у всех видимых RP — конвенция hide-self (как QPILL)
     формально совместима, проверить на 2B нельзя (не читается).
   - 2A.0x190=0 (x4x4x4x4), 1A=1, 3A=2 — как известно; +0xC0-режим:
     1A=2, 2A=3, 3A=3; +0x490=0x0F0F0000 у 2A/3A, у 1A нет.
4. **Дампы QPI-видимых** (FF:0B.0/0B.1/0C.0/0D.0/1F.0): таблицы
   роутинга R3QRT (значения совпадают с трейс-записями), сид-хэши
   0x81E4C249; маски скрытых портов {1B,2B,2C,2D,3B,3C,3D}
   (0x754/0x3A2 в 11/10-битной нумерации) НЕТ. Кандидат-любопытство:
   0B.0+0xC0/+0xD0 = 0x11111164 (ниблы).
5. **Пробные записи-анхайды** (безопасные нули, мастер-аборт
   безвреден): 2B+0xFC:=0 и QPILL0(FF:08.0)+0xFC:=0 — **обе no-op**
   (VID остался FF): скрытие = decode-off, записи не декодируются.
   ECAM-дверь закрыта с обеих сторон.
6. **00:00.0 host bridge полный дамп**: +0x1A0=0 (финал машины
   состояний из трейса), масок нет.

**Вывод §T**: скрытие — настоящий decode-off на уровне ункора;
ни в одном видимом конфиг-пространстве маски/гейта не найдено.
Следующий живой фронт — **PECI-путь** (даташит E5v2: DEVHIDE-скрытые
устройства доступны с SMBus/PECI-порта IIO): на риге есть AST2400
BMC с PECI (009_AST2400PeiInit в FV12) — пробовать транзакции
RdPCIConfig/WrPCIConfig к скрытым 2B/QPILL через BMC (IPMI OEM
raw, netfn 0x30-0x3x), читать DEVHIDE-состояние и писать анхайд
мимо ECAM-декода.

### U. Фронт DEVHIDE/PECI: datasheet-раскол, кандидат-отказ, инструмент-уроки (2026-09-17 ночь→)

Порядок работ (ответ на вопрос хозяина): сначала поиск (деск +
read-only пробы), «жечь» (флеш патча) и лог бута — только если
рантайм-пути не взлетят; для PECI/регистровых проб флеш не нужен.

#### U.1. Datasheet E5-2600 v4 Vol.2 (cdrdv2-public.intel.com/333810, публичный)

1. **§1.2.1.5 Device Hiding**: механизм скрытия существует
   архитектурно — «all PCI configuration accesses … from Intel QPI
   will be master aborted … implemented via the **DEVHIDE**
   register»; «BIOS must hide all unused RPs devices in IIO config
   space, via the DEVHIDE register **in Intel QPI Configuration
   Register space**» (примечание к SLTSTS presence_detect_state).
   Определение регистра (device/offset/биты) в публичном Vol.2
   E5v4/E7v4/D-1500 ОТСУТСТВУЕТ (только проза; NDA EDS). E5 v2
   (#329188) определение содержит — PDF недоступен (Akamai-гео,
   wayback 429; снипет корейского зеркала подтверждает наличие
   R3QPI-главы с DEVHIDE).
2. **pcie_iou_bif_ctrl = B0:D{1,2,3}:F0+0x190** (§6.2.86): биты 2:0
   — селектор бифуркации (наш 0x190!), бит 3 — start strobe.
   Dev2/Dev3: 000=x4x4x4x4, 001=x4x4x8, 010=x8x4x4, 011=x8x8,
   100=x16. Таблицы 6-2/6-3/6-4 «Function# of Active Root Ports»:
   при x4x4x4x4 активны **все 4 функции** (мапа 3,2,1,0) ⇒
   2B/2C/2D = implemented-active, спрятаны именно DEVHIDE'ом,
   а не отключены бифуркацией. Live-селекторы: 1A=1(x8),
   2A=0(x4x4x4x4), 3A=2(x8x4x4).
3. §6.1.3: конфиг-доступ к IIO возможен «over message channel or
   directly via the JTAG mini-port» — так сайдбанд (PECI/JTAG)
   обходит ECAM-декод — базис PECI-фронта.

#### U.2. Кандидат R3QPI0+0xC0/+0xD0 — ЗАКРЫТ живым флип-тестом

Значение 0x11111164 @ FF:0B.0 (DID 6F81, R3QPI0) +0xC0 и +0xD0
(единственный не-лейновый кандидат; шесть единиц ≈ шесть скрытых
реализованных функций старой раскладки: 2B/2C/2D+QPILL0/1/2):

1. Дифтест раскладкой (3A: x16→x8x4x4, т.е. 3B/3C стали
   implemented+hidden): значение НЕ изменилось — гранулярность
   мапы не пер-функциональная (или регистр не тот).
2. **Обратимый флип-тест** (pread/pwrite /dev/mem, try/finally):
   регистр RW — запись читается назад идеально; обе копии =0
   1.5 сек ⇒ **2B/2C/2D/3B/3C/QPILL0 все ещё FFFFFFFF** — видимость
   не гейтится. Восстановлено 0x11111164 в обе копии, инварианты
   целы (2A=6f048086). Вывод: это НЕ DEVHIDE-гейт видимости.
3. Трейс-аудит: в v22-логе НУЛЬ обращений к 3F/FF:0B-0D+0xC0/0xD0
   — писца в трассируемой вселенной нет; литерал 0x11111164 в
   корпусе (62 PEI + 207 DXE + nat-uncore) отсутствует.

#### U.3. Живые дампы-дополнения (шина 0)

- **00:05.0/05.1/05.2 полные**: 05.0 (UBOX Map/VTd/SM):
  mmcfg_base@+0x90=0x80000000 ✓; +0x180-0x1B0 = VT-d
  (vtbar/vtgenctrl/vtisochctrl/vtuncerr*) — +0x1B0=0x1bf это
  vtuncerrsev, НЕ хайд-мапа; +0x1C0=0x42030170 (UBOX+1C0 из
  трейса — да, это bus0 05.0). 05.1 (Hot Plug): порт-капы
  +0x100-0x13C. 05.2 (RAS/CS): error-status'ы. Хайд-масок нет.
- **00:00.0 полный** (56 ненулевых): VT-d/vendor-капы; масок нет.
- busff.json 0C.0-7/0D.0-1: значения +0xC0-0xE0 = лейн-таблицы,
  буква-в-букву совпадают с 3F-трейс-записями QPI-инита
  (0x0C.5+0xD8=0x207 и т.д.) — двойное отображение ункора живо
  и в ОС (FF), подтверждено кросс-сверкой.

#### U.4. PECI-фронт: разведка

- BMC (TSM): IPMI 2.0, FW 2.36, Manufacturer ID 0 (безымянный
  OEM), Product 502; `sdr type Temperature`: **CPU1 Temp = 42°C
  live** — BMC непрерывно ходит PECI к сокету 0, петля физически
  жива (CPU2=0 — 1S, корректно).
- Wire (libpeci/openbmc): RdPCIConfigLocal = **0xE1**,
  WrPCIConfigLocal = **0xE5** (PECI 3.0-набор; legacy 0x14/0x15);
  поля отдельные bus/device/function/reg(u16); длины 1/2/4.
  Цитата док-коммента: покрывает «all processor IIO and uncore
  registers … even before BIOS enumeration» — ровно наш случай.
- OEM-пасстру на TSM 2.36 НЕ найден: `raw 0x3a 0x31` (фан-дьюти,
  находка RD650) = **C1 команды нет**; `0x32 0x91` = Nuvoton-PWM,
  не наш случай; Lenovo Grantley SNMP/MIB UG (pccbbs, 19 стр.) —
  только стандартный netfn 0x04; AMI/Intel паблик-спеков нет.
- Следующий шаг: **TSM-бандл** (support.lenovo.com ds102390,
  pccbbs-CDN) — статику BMC-прошивки грепать на обработчик
  PECI-пасстру; страница/API для curl 403 (Akamai) — хозяин
  скачает браузером.

#### U.5. Инструмент-уроки (/dev/mem)

1. `m[a&0xfff:a&0xfff+4]` — приоритет операций даёт `a&0x1007`:
   булк-слайс → нули/стена-FF (ложные «нулевые» чтения).
2. mmap slice-присваивание в конфиг-пространстве может кинуть
   OSError/EINVAL, ЧАСТИЧНО прошив запись (наблюдено: c0 стал 0
   при «упавшем» войте) — только `os.pread/os.pwrite`.
3. Эксперименты с регистрами — всегда try/finally-восстановление
   + контроль-аборт по baseline (assert до записи).

**Вывод §U**: DEVHIDE — реальный механизм (архитектурно
подтверждён), но его регистр не в видимом R3QPI0+0xC0/0xD0;
в видимых конфигах шины 0 и FF масок больше нет. Два оставшихся
пути к скрытому пространству: (1) **PECI-пасстру через BMC**
(нужен TSM-бандл для статики), (2) точное определение DEVHIDE из
E5 v2 #329188 (нужно зеркало PDF). Если PECI-чтение 2B оживет —
...
#### U.6 NB. Живая проверка раскладки «все мелланоксы» (2026-09-22)

Хозяин переселил карты: бут-флеш → USB (бут с USB проходит сам,
без вмешательства), во все слоты — Mellanox'ы (все x8): ConnectX-3
(15b3:1003) за 2A (03:00.0), ConnectX EN 10GigE (15b3:6750) за 3A
(04:00.0), ConnectX IB (15b3:673c) за чипсетным 1c.4 (08:00.0).
Ответ на вопрос «почему наш слот x8-карту показывает x4»:

- **2A: LnkSta Gen3 x4 (downgraded) — живое следствие BIOS-
  настройки x4x4x4x4** (селектор 00:02.0+0x190=0), НЕ поломка и
  не «починилось»: в x4x4x4x4 порт 2A владеет только лейнами 3:0,
  остальные 12 — у скрытых 2B/2C/2D. Т.е. бифуркационная половина
  цели в железе РАБОТАЕТ (LnkSta x4 = селектор реально применён);
  отсутствует только публикация функций.
- **Чипсетный 1c.4: LnkCap x4** — электрически x4 слот, x8-карта
  честно даёт x4 downgraded. Железо, не настройка.
- **3A: селектор теперь 4 (x16)** — RC снова сам пересобрал
  раскладку под население слота (история: 4→2→4); порт владеет
  всеми 16 лейнами, ConnectX EN тренируется честным **x8 Gen2**.
  1A=1 (x8) с I350 за ним — без изменений.
...
#### U.7-erratum. Видимость 2B/2C/2D в Setup — НАШ ЖЕ ПАТЧ v15, не сток (2026-09-22, хозяин поймал)

Замечание хозяина: «это же ты сделал еще в этой сессии» — верно.
Проверено движком (uefi-cli hii form gates) на трёх образах:

- **сток** (HNX99TF_200525_original): ВСЕ формы портов 85-93
  (1B/2A/**2B**/**2C**/**2D**/3B/3C/3D) — suppress
  `checkbox(qid) == 0x0001` (qid-цепочка 0x376/0x3AA/0x3DE/0x412/
  0x446/0x4BB/0x4EF/0x523; чекбоксы «порт отсутствует» на
  смежных varstore-офсетах 0xc50+ в IntelSetup, seed=1,
  перезаписываются RcSetup-коллбеком). В стоке подменю скрытых
  портов **невидимы**.
- **v14**: гейт 2B ещё сток (`0x3BF == 0x0001`).
- **v15-unhide-2bcd**: гейты 87/88/89 (2B/2C/2D) — константа
  заменена на **0xFFFF** (неактивируемый → формы всегда видимы);
  остальные порты не тронуты. Патч едет в кумулятиве до залитого
  v22 → на риге подменю 2B/2C/2D видны именно поэтому.

Что из §U.7 остаётся стоковым: IOU0=[Auto]→x4x4x4x4; «Max Width
x16» = дисплей LnkCap; help-текст механизма удаления; чтение
статусов скрытого пространства коллбеком («Link Did Not Train»);
вопрос **«Hide Port?»** (one_of no/yes, IntelSetup offset 0xbb8)
и «Hot Plug Capable» на форме 2B. Побочная находка: чекбоксы
0xc50+ = пер-портовый presence-флаг в IntelSetup, который читает
сток-гейт — Setup-зеркало presence-цепочки.

#### U.8. Эксперимент Hot Plug Capable=Enable на 2B/2C/2D — МИНУС по анхайду, Setup-слой закрыт (2026-09-22, «делай конечно»)

По help-тексту формы («BIOS will remove the EXP port if no device
… and the device is not HP capable»): HP-capable порт не удаляется
авто-режимом. Выполнено: Setup через SOL, на всех трёх формах
(87/88/89) Hot Plug Capable [Disable]→[Enable] (клавиша «+»),
F10 Save&Exit, бут.

1. **Вердикт: ECAM-декод НЕ раскрылся** — raw /dev/mem после
   бута: 2B/2C/2D VID/DID = FFFFFFFF, селектор 2A+0x190=0 как был;
   lspci только 00:02.0. Вместе с уже стоявшим PCI-E Port=[Enable]
   (2B) — **Setup-слой (политики RcSetup) закрыт как рычаг**:
   удаление-по-help и аппаратный DEVHIDE — разные механизмы,
   DEVHIDE ставится ниже (RC PEI / страпы).
2. **Побочные эффекты (дельта поведения RC)**: (a) дамп
   DumpIioPcieLinkStatus начал итерировать **все 4 функции dev2**
   — F[1]/F[2]/F[3] отдельными строками («Link Down!»), раньше
   печатался только живой F[0] (+фантомный F[1] в v22-max-трейсе);
   (b) фантомный тренинг 2B «Link up as x04 Gen3» (верхние лейны
   x8-карты на лейнах 7:4 порта 2B) исчез и **не вернулся после
   отката** HP=Disable — либо sticky-эффект ункора от HP-бута
   (тёплые ресеты не чистят sticky-биты), либо дельта NVRAM: F10
   сохраняет ВСЕ вопросы формсета, включая бывшие рантайм-значения.
   Diagnoстическая ценность фантома (лейны 2B доведены до слота)
   уже зафиксирована ранее (§P/Q).
3. **Откат выполнен**: HP=Disable на всех трёх, F10, бут, базлайн
   зелёный (2A LnkSta Gen3 x4, три Mellanox'а на местах, только
   2A в lspci). Машина в ОС.

Остающиеся фронты не изменились: PECI-пасстру через BMC (ждёт
AC-cycle хозяина) и точный офсет DEVHIDE по E5 v2 datasheet
...
#### U.10. Сестринская матю (pigoky, R450X219) — СТОК = x16 на слоте 1; мой «сток-вердикт» §U.6 ОТКЛОНЁН (2026-09-22)

Хозяин дал read-only SSH на сестринскую RD450X (BIOS **R450X219**
11/18/2016 — сток, старее нашей R450X231; там 2×RTX 3060 + LLM).
Цензус (только lspci/setpci-чтения):

1. **2A (00:02.0): селектор +0x190 = 4 (x16!), за ним GA104 RTX
   3060 — LnkSta x16 Gen3.** 3A=4 (x16, вторая 3060), 1A=1 (x8).
   Функции 00:02.1-3/00:03.1-3 **не существуют** (в x16-режиме
   селектора сиблинги не реализованы вообще — не «скрыты», их
   нет; §U.9-категория «turned off» рядом не стояла).
2. **Вердикт по спору «слот всегда x16»: хозяин ПРАВ.** Сток с
   x16-картой держит слот 1 в x16. Моё §U.6-утверждение «x4x4x4x4
   — сток-дефолт dev2» (базировалось на статике §P «кодовые
   дефолты nat {0,4,1}») — **неверно для живого стока**: RC
   перекладывает селектор под население (живой сток = {4,4,1}).
   Ответ на исходный вопрос хозяина «что мы сделали, что x8-карта
   тренируется x4»: **наше v-наследие** — v6-bif-4x4x4x4 →
   v17-baked-defaults → вшитый дефолт бифуркации 0 едет в
   актуальной v22. Перестановка карт местами картину менять и не
   могла (селектор — сторона порта), «разные делители» в Setup
   поверх baked-дефолта — перекладывались/не приживались.
3. Побочное: **R3QPI0 +0xC0/+0xD0 = 0x11111164 на стоке тоже** —
   значение стоковое (флип-тест-отказ §U.2 корректен и для
   стока); QPILL 7f:08.0 на сестре ВИДИМ (на нашей FF:08.0 скрыт
   force_l1 — у сестры QPI-линки, похоже, в деле).
4. **Следующий шаг (предложение хозяина принято): вшить родной
   сток-дамп** `refs/fw/HNX99TF_200525_original_E5C88C6F.bin`
   (наш собственный дамп R450X231) на нашу матю → F9 → селектор
   под нашим рейзером/мелланоксом: 2A=4 ⇒ расщепление было чисто
   нашим артефактом; 2A=0 ⇒ сток сам расщепляет под NVMe-рейзер
   (VPP-детект). Заодно чистая база без v-наследия (включая
   v15-setup-unlock) для финального PECI/DEVHIDE-этапа.

#### U.11. Сток вшит, цензус живой: 2A=4/x16, ConnectX-3 x8 Gen3 — РАССЩЕПЛЕНИЕ БЫЛО НАШИМ АРТЕФАКТОМ (2026-09-22)

Хозяин прошил родной дамп (R450X231 02/07/2018 подтверждён в
ОС). Цензус read-only (без ребута, по просьбе хозяина):

1. **Селекторы: 1A=1 (x8), 2A=4 (x16), 3A=4 (x16).** За 2A
   ConnectX-3 (x8) тренирует **честный x8 Gen3**; за 3A ConnectX
   EN — x8 Gen2. Сиблинги 2B/2C/2D/3B/3C/3D не существуют (x16-
   режим селектора их не реализует — lspci только fn0).
2. **Вердикт по §U.10-развилке: 2A=4 ⇒ x4x4x4x4-расщепление на
   нашей матю было ЦЕЛИКОМ нашим v-артефактом** (v6-bif0 →
   v17-baked-defaults). Сток НЕ расщепляет под нашим рейзером
   (VPP-гипотеза отклонена). История «фантомного 2B-x04» = верхние
   лейны карты в НАШЕМ split-режиме. RP+0xA0: 1A/2A=0x0C40,
   3A=0x0C00 (3A без 0x40 — сток-поведение, подтверждение §T).
3. Ункор-шина на стоке = FF (как при v22); **R3QPI0+0xC0/+0xD0
   = 0x11111164** — тройное подтверждение стоковости (сестра +
   v22 + сток-наша).
4. Сток-базлайн сохранён: `hack/stock-r450x231-bus0-dumps.txt`
   (полные 4KB 2A/3A/05.0, ненулевых 72/73/50).

**Итог-пересборка охоты**: цель «получить x4x4x4x4» не была
сток-функцией — это наша правка; сток даёт x16/x8-порты. Финальная
цель формулируется заново: **на стоке выставить Setup-селектор
IOU0=x4x4x4x4 (без прошивки!) и найти, почему 2B/2C/2D не
экспонируются** — теперь это чистый эксперимент без v-наследия.
Следующие шаги: (1) Setup → IOU0 → x4x4x4x4 на СТОКЕ (проверить,
что селектор реально применяется и что видит ОС — сток-писец
DEVHIDE может повести себя иначе); (2) PECI-фронт после AC-cycle
хозяина (§W.4, V3_24 прошивка BMC — без неё движок мёртв);
(3) при необходимости — дифф сток-vs-v22 дампов (§U.11-артефакт).
