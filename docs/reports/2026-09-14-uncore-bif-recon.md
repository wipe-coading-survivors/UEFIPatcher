...
32868 байт @ смещение 0xBDD9F8 (0-based 12428136 — выровненный конец
файла `8/242` внутри DXE-FV); изменено 31936 байт, остальные 932 байта
спана — 0xFF-паддинг хвоста артефакта, совпавший с филлером свободного
места FV; вне спана образ побайтово равен базе. Единственное отличие
...
>    - `BIF-EPA: prev stage: none (…)` — на первом буте (NVRAM-переменной
>      ещё нет); `BIF-EPA: prev stage=0` — на последующих
