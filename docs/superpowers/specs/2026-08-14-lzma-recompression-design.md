...
> **Поправка (2026-08-14, коммит `8160c5e`).** PE-resident IFR-патчинг
> реализован в тот же день: третий гейт сужен (PE32 с FORMS-пакетами в
> 'HII'-ресурсах мутабелен, in-place unsuppress + форм-дискриминатор
> `#<form_id_ifr>`); ожидание real-image теста перевёрнуто на полный
> round-trip. Детали — поправка в спеке фазы 6 §6.
