...
- Emit кросс-формсетного GOTO = REF3 (QuestionId = 0xFFFF =
  ...
  [Исправлено 2026-09-22: живой гейт positional-insert показал, что
  EDK2-маркеры (vsid 0xFFFF, QuestionId@15 0xFFFF) живым AMITSE не
  рендерятся; родные REF3 этого образа несут нули и последовательные
  ненулевые qid — `emit_ref3` переведён на нативный AMI-паттерн
  (vsid 0, QuestionId 0). Дифф — аддендум «Живой гейт п.3» спеки
  positional-insert.]
...
|--------|-------------|--------|-------|
| 0x247 | 118 (IIO 0) | 0x539 | 0=x4x4, 1=x8, 0xFF=Auto |
| 0x248 | 118 (IIO 0) | 0x531 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x249 | 118 (IIO 0) | 0x535 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x25c | 119 (IIO 1) | 0x53a | 0=x4x4, 1=x8, 0xFF=Auto |
| 0x25d | 119 (IIO 1) | 0x532 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x25e | 119 (IIO 1) | 0x536 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x270 | 422 (IIO 2) | 0x53b | 0=x4x4, 1=x8, 0xFF=Auto |
| 0x271 | 422 (IIO 2) | 0x533 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x272 | 422 (IIO 2) | 0x537 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x284 | 423 (IIO 3) | 0x53c | 0=x4x4, 1=x8, 0xFF=Auto |
| 0x285 | 423 (IIO 3) | 0x534 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |
| 0x286 | 423 (IIO 3) | 0x538 | 0=x4x4x4x4 … 4=x16, 0xFF=Auto |

Раскладка на IIO x (x = 0..3): порт-бифуркация (x4x4/x8) @ `0x539+x`
(qid 0x247/0x25c/0x270/0x284), полная бифуркация @ `0x531+x`
(0x248/0x25d/0x271/0x285) и @ `0x535+x` (0x249/0x25e/0x272/0x286).

> **Fix 2026-09-13 (живой `hii question info`, оба образца):** qid
> зависят от образа (формы и офсеты идентичны, qid нет):
> `refs/amibcp/450x — копия.bin` (родная база владельца) — qid
> 0x242/0x243/0x244 (ф.118), 0x257/0x258/0x259 (ф.119),
> 0x26b/0x26c/0x26d (ф.422), 0x27f/0x280/0x281 (ф.423) — исходная
> таблица §6 была верна ДЛЯ НЕЁ; `refs/fw/HNX99TF…bin` (pristine,
> «тихий» hyperscale-билд) — 0x247/0x248/0x249, 0x25c/0x25d/0x25e,
> 0x270/0x271/0x272, 0x284/0x285/0x286 (таблица выше). Вопрос
> владельца «IOU0 (IIO PCIe Port 2)» = 0x243@0x531 (копия) /
> 0x248@0x531 (HNX); прочие экземпляры той же страницы —
> 0x258@0x532 / 0x26c@0x533 / 0x280@0x534 (копия). Шаблон копии
> нёс следы AMIBCP-эпохи владельца: 0x531=04 (x16), 0x532=03
> (x8x8). NPP-вопросы живут вне бифуркационной зоны (HNX:
> 0x259@0x1758; на копии 0x259 — «IOU1 (Port 3)», коллизия имён).
...
Вердикт: **PASS — 2026-09-13, владельческий железный гейт на 450x пройден.**

### Вердикт-аддендум (владелец, 2026-09-13)

Итоговая рабочая схема (история подбора — ниже):

- Чистая копия образа + единственный add-ref в форму **Advanced 10002**
  корневого Setup (`899407D7-…:0x10:0#10002`), refs-запись:
  `{ "form_id": 1, "prompt": "Intel RC Setup", "help": "Intel RC Setup
  Configuration", "question_id": 0, "formset_guid": "EC87D643-…" }`
  → `:image save` → прошивка.
- Железо: Advanced содержит пункт «> Intel RC Setup»; Enter открывает
  полный корень IntelRCSetup (RC Revision 04.00.00, Processor/QPI/
  Memory/**IIO**/PCH/Misc Configuration, Server ME, Runtime Error
  Logging, Reserve Memory) — дерево с вложенностью, бифуркационные
  формы 118/119/422/423 достижимы через IIO Configuration.
  Evidence: копипасты серийной консоли (Aptio Setup Utility, 2026-09-13).

Почему не #10008/#10000 (попытки владельца 2026-09-12, обе прошиты):

- **#10008 (Chipset)** — тупик: сам GOTO→10008 в корневой форме 10000
  закрыт `suppress_if TRUE` (`@pkg+0x6da`, класс ConstantTrue — планер
  не флипает, TODO); страница недостижима, пункт на ней не виден.
- **#10000 (корневая, qid 32800)** — REF3 не отрисовался вкладкой бара:
  бар AMITSE строится из qid-0 GOTO-детей формы 10000 (6 шт: 10001/10002/
  10008/10009/10010/10012; «Server Mgmt» TSE дорисовывает извне от
  IPMI-приложения); REF3 с ненулевым qid вкладкой не принимается.
  [Уточнение 2026-09-22, инструментальный гейт positional-insert:
  барные GOTO формы 10000 несут qid 1..6, а не 0 — «qid-0 GOTO-дети»
  было неверной читкой; факт «REF3 qid 32800 вкладкой не принялся»
  остаётся, требование qid 0 — гипотеза до прошивки артефакта
  `450x-positional-intelrcsetup.bin` (e22bd0dd…).
  Финал 2026-09-22: артефакт прошит — qid-0 REF3 вкладкой ТОЖЕ не
  принят; гипотеза qid 0 снята. Полевой дифф: наш REF3 нёс EDK2-
  маркеры vsid/QuestionId 0xFFFF, родные REF3 (рендерятся) — нули;
  следующий кандидат — мимикрия нативного паттерна. Детали —
  аддендум «Живой гейт п.3» спеки positional-insert.]
- Работающий образец нашли в самом образе: оригинальные REF3
  Advanced→IntelRCSetup#2/#4/#12 и Main→#1793 (4 из 5 негейтованы) —
  «Processor Configuration» в Advanced рендерится на живом BIOS, что
  одновременно доказало: формсет EC87D643 зарегистрирован в HII на
  рантайме. Рецепт выше — точная имитация этого паттерна (тот же хост
  10002, qid 0, цель #1).

Хвосты (TODO.md): вкладка топ-уровнем в бар (позиционный insert +
эксперимент qid 0 в 10000), класс ConstantTrue (TRUE→FALSE байт-флип,
раскроет Chipset), extract/export+move форм, валидация цели add_ref.
