...
### §7.1 Таблица занятости (первый прогон зонда: 2026-09-19, 450x, RC-форма 1)

Живая карта RC-формсета `ABBCE13D-E25A-4D9F-A1F9-2F7710786892:0x10:0` —
ровно две декларации: id 1 = IntelSetup (EC87D643-EBA4-4BB5-A1E5-3F3E36B20DA9,
size 0x1670), id 2 = AmiSetupSupportedFeatures (EC87D643…, size 4).

| id | статус | определение |
|----|--------|-------------|
| 21 | free | — (нет в карте формсета) |
| 22 | free | — |
| 23 | free | — |
| 24 | free | — |
| 25 | free | — |
...
Зонд (`check_question_add` varstores-параметром) дал Ok(()) на всех десяти;
кросс-чек против карты §3 сошёлся (занятость ⟺ присутствие в карте).
В диапазоне 21–30 занятых нет (в формсете заняты только ids 1–2); ветка
«busy» зонда валидирована ассертом на заведомо занятом id 1 внутри теста.
