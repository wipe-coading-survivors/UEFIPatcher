...
- `real_image_hii_set_value_matches_e14` — set_value(1) → build:
  ...
  и эквивалентен эталону E14 конструктивно (эталон-файл вне репо:
  тот же базовый образ + ровно однобайтовый диф декомпрессата
  [(0x66, 0→1)]); флеш-дифф против оригинала только в [0x8000C2] ∪
  экстенте секции 9221315B; re-parse:
