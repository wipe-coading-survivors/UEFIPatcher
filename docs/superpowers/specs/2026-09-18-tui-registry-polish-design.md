...
вообще мертвы. Остаток V3-мелочей: `fmt_u32_ids` и ассерт
...
7. R8 (скролл details-панелей) добавлен по ревью спеки: Forms View —
   ...
   с оживлением мёртвого фокуса Details. Список форм/строк скроллится уже
   сейчас (stateful List) — дыра была только в details-панелях.
...
- TestBackend-рендер: бейдж R/W в строках образов, полный UUID выбранной
  ...
  основной вид — j/k в Details focus меняет скролл.
