...
| UncoreInitPeim натив | извлечь из базы (GUID начинается с `a48b1cd7`, точный — снять `node search` на R0; размеры трёх билдов 606322/628434/627410, маппинг снимается на R0) | главный предмет дизасма |
...
| Розетта `IIO-pei32.bin` | `refs/fw/IIO-pei32.bin` (798880, PE32, четвёртый билд) | строковые якоря «Invalid IOUx Bifurcation» / «busUncore» / «busIio» — локализатор кода бифуркации |
...
обёртки MmioWrite), интерактивный дизасм диапазона. Отрабатывает и
на розетте (строки есть) — метод калибруется на ней, потом
натягивается на натив/HNX (там строк может не быть — ищем по
структурной похожести кода вокруг тех же вызовов).
...
(`mmr_table.json` рядом с отчётом) — пробник **не имеет** режима
...
(паттерн SerialDxe: 8N1 115200, базу — из R0/R2) — печать best-effort,
на готовность чужих SerialIo не завязываемся. NVRAM-переменная `BifEpaStage` (gRT->SetVariable,
BOOT_SERVICE_ACCESS) — маркер прогресса и **one-shot защита от
ресет-лупа**. Телеметрия двухканальная: UART (базу даёт R0/R2-разведка
натива; SOL-путь жив) + сама переменная (гарантированный канал —
каждый следующий бут пробник первым делом печатает
`previous stage result` из неё; UART-молчание при зависании не
теряет данные).
...
| UART-телеметрия молчит | NVRAM-канал self-report следующим бутом; EPA-1 специально проверяет телеметрию до любого write |
