...
- `plan_ref_step(refs, body, form_add_outcome, busy_qids)
  ...
  `max(0x7F00, max(busy_qids)+1)`. Высокие id — конвенция
...
6. **Varstore-контракт**: refs-only путь varstore-free по построению (REF-
   ...
   экспорт varstores не заполняет (walker не собирает, TODO:490) — известное
   ограничение: round-trip пакет в чужой формсет может ссылаться на
   несуществующие `var_store_id`, защита до закрытия TODO:490 — только
   warn шага 5; per-item pre-check заведён отдельным пунктом TODO.
