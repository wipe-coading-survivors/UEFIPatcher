...
Развилки плана C:
- **C-тест (живой, дёшево)**: подтвердить на риге пару
  ServerSetup∈NVRAM / IntelSetup∉NVRAM (дамп NVRAM-региона);
  попутный ориентир — на asr1 переменные EC87D643 живут
  (asr1-live1-Setup-ec87d643.bin): есть ли там IntelRCSetup
  топ-уровнем?
- **C1**: добиться создания переменной IntelSetup (RE драйвера
  ABBCE13D: кто зовёт SetVariable; baked-дефолт уже в образе).
- **C2**: TSE-патч — обойти проверку существования переменной
  для EC87D643-записи блока 3 (нужен дочитанный дизасм
  потребителя 0x2c3xx и/или поиск вырезанной ветки).
