...
- **Карве-аут qid 0 для refs**: коллизия-чек `check_ref_slots`
  (`hii/mod.rs:1897`) сегодня отвергнет qid 0 («already exists») — а в
  10000 все 6 барных GOTO уже qid 0. Qid 0 — навигационный (без
  NVRAM-хранилища), дублирование легитимно по живому образцу; для refs
  разрешаем, для storage-вопросов `add_question` запрет остаётся (qid
  адресует `$SPF`-запись).
...
- **Карве-аут qid 0**: `check_ref_slots` (`:1897`) и коллизия в
  `check_question_slots` (`:1475`) — при `schema.question_id == 0` в
  refs-ветке коллизия не проверяется (навигационный qid, живой образец
  — 6 qid-0 GOTO в одной форме). Для `QuestionAddSchema` (storage-вопросы)
