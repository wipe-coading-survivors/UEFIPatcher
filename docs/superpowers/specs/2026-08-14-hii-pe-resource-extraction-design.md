...
`collect_strings`: ресурсный канал становится достижимым для поиска
строкового пакета; семантика «первого пакета» сохраняется (не агрегируем
все списки: на HNX99TF вторичные пакеты имеют язык `x-UEFI-AMI` —
агрегация сломала бы ассерт `en*`). Язык читается уже корректным кодом
@+46.
...
каналов таргетят PE32-секцию) нашёл бы мусорный «SUPPRESS_IF-скоп» в
...
пункту спеки. (Суперседируется поправкой фазы 7 выше — ожидание изменено
на `NotASetupItem`.)
...
- `tests/fixtures/hii_rk3588_string_res.bin` — 'HII'-ресурс-блоб
  (6 876 B, список `A487A478-51EF-48AA-8794-7BEE2A0562F1`, STRING
  6 852 B, язык en-US; PE@0x58e304, блоб @0x598374 распакованного
  LZMA-буфера) — ресурс-канал + реальный строковый заголовок;
- `tests/fixtures/hii_rk3588_bare_form.bin` — bare FORM-пакет
  (1 424 B, formset `642237C7-35D4-472D-8365-12E0CCF27A22`, 21 форма;
  PE@0x22e26c +143588 → 0x251350 в распакованном буфере) — bare-канал.
  Ранее указанный здесь `5DAF50A5` (2 186 B) найден probe только в
  OVMF/капсуле — атрибуция исправлена при написании плана;
- PE-обёртки: минимальный синтетический PE32+ с .rsrc 'HII' собирается
  прямо в тестах `pe_resource.rs`; двоичная PE-фикстура не коммитится
  (решение зафиксировано при написании плана, задача-0 свелась к
  извлечению двух блобов выше);
