...
### 3.5 `scope_balance` → pub(crate)

Хелпер баланса скоупов IFR поднимается в `hii::ifr` как `pub(crate)`; локальные
копии в тестах `mod.rs` и `real_image.rs` удаляются (инвариант железо-доказан
раундом 8 дуги setup-new-page).
...
- CLI: `hii form set-visibility` — тексты новых ошибок; e2e/mock-тесты обновить;
- gateway: `/set-visibility` — теперь 404/400 вместо 200-пусто; integration-тесты
  обновить;
