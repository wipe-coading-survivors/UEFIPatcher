...
- **Walker на AMI-образах:** IFR-специфика уже выверена в `values.rs`/`gates.rs` (quirk-маски,
  bounds) — walker V1 обязан строиться на том же фундаменте, не новом парсере. Риск регресса
  низкий, но реальный: юнит-фикстуры из живого образа обязательны.
...
- **D3:** Единственная engine-добавка дуги — `HiiListQuestions` (ленивый, per-form); `FormInfo` не
  расширяется.
- **D4:** Strings-браузер — режим внутри Forms-view (`S`), не третья вкладка: строки — часть
  логического HII-слоя; если понадобится образ-уровень (несколько языков параллельно) — вынесем
  вкладкой позже.
