...
- `output.rs` tests-mod: кейс `print_question_info` с defaults — text-формат
  содержит строку дефолта (значение + varstore-адрес), tsv — колонку.
  Точный вид строки — из текущей реализации принтера (RED по отсутствию
  покрытия, не по изменению вывода).
...
- ассерты ставятся на существующие инвокации; новые CLI-вызовы не добавляем
  (кроме кейсов §3.3 — это exit/stderr-ассерты).
