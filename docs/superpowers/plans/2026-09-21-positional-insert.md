...
- `preflight_question_splice(image, target, bare_channel, form_id, pos, ops_len, strings)`: bare-ветка `:1417-1419` → `ifr::locate_insert_at(&node.body, 0, form_id, pos).map(|_| ()).ok_or(HiiError::NotFound)`; resource-ветка `:1431` → `check_rsrc_question_splice(&post_strings, form_id, pos, ops_len)?`
