...
- Числ. факты (верифицированы живьём, спека §«Контекст» + брейншторм):
  - 226D2IL3.30/.50: стор = FFS Raw `CEF5B9A3` @0x500048 (FV@0x500000),
    ...
    204 байта (3.30) / 206 байт (3.50 — живой замер Task 12; план считал
    одинаковыми); 14 записей во вложенном сторе (GUID-стор 0x60/6 GUIDs и
  ...
  - C275D4I3.20: во вложенном сторе есть `Setup` 226b, `IntelSetup` 608b,
    `ServerSetup` 456b (живые замеры Task 12: цепочка записей сходится в
    конец StdDefaults 0x508+479=1950; старые 215/597/445 — из TODO:1499 без
    пересчёта, как и 226D2I 1206/195 той же строки, исправленные на +11/+9).
  - Оба образа несут ВТОРУЮ запечённую копию StdDefaults за
    non-recompressable Tiano (226D2IL: 2007b, C275: 1982b; те же
    переменные) — легитимно метится «NVRAM store» (body-probe §2, без
    барьера — как HNX FV2-LZMA копия), в nvar list/set не входит
    (пропуск барьера, план-фикс c84c6bb); гейт list_items ждёт 2
    узла-стор у 226D2IL.
...
Сразу после добавления поля добить исчерпывающие литералы `Node { … }`
без `..Default::default()` в зависимых крейтах — иначе E0063
(missing field `is_nvar`), `cargo test --all` падает:

- `crates/uefi-cli/src/output.rs` (тест `node_serializes_type_field`)
- `crates/uefi-cli/tests/mock_server.rs` (`image_nodes_list`)
- `crates/uefi-gateway/tests/mock_server.rs` (`image_nodes_list`)
- `crates/uefi-tui/src/tree.rs` (тест-хелпер `nd`)
- `crates/uefi-tui/tests/mock_server.rs` (`image_nodes_list` — 5 литералов)

В каждый литерал добавить `is_nvar: false,`; проверка полноты списка:
`cargo check --workspace --all-targets` (ревизия Task 3: первая редакция
списка пропустила uefi-tui и была дополнена по E0063-сводке воркспейса).
...
```bash
git add crates/uefi-proto/proto/engine.proto crates/uefi-engine/src/nvar.rs crates/uefi-engine/src/parser/image.rs crates/uefi-cli/src/output.rs crates/uefi-cli/tests/mock_server.rs crates/uefi-gateway/tests/mock_server.rs
...
- Produces (потребляют Task 6 RPC, Task 8 CLI, Task 12 гейты):
  ...
  - `pub(crate) fn collect_stores(node: &FfsNode, path: &mut Vec<usize>, barrier: bool, file_guid: Option<&Guid>, out: &mut Vec<StoreNodeHit>)` (стор за non-recompressable секцией пропускается, не отказ)
...
```rust
    ...
    fn nvar_list_skips_store_behind_hard_compression() {
        ...
        let out = nvar_list(&img, None, false).unwrap();
        assert_eq!(out.len(), 1, "запечённая копия за non-recompressable секцией не листится");
...
В `nvar.rs` (импорты: `use crate::types::{FfsNode, FfsType, Guid, Image};`):
...
- Produces (потребляют Task 6 RPC, Tasks 12-13 гейты):
  - `#[derive(Debug)] pub struct NvarSetOutcome { pub applied: Vec<String>, pub stores: Vec<String> }` (Debug обязателен: тест Step 2 зовёт unwrap_err → T: Debug)
...
Mixed-тест (живой стор + запечённая копия за барьером — правка применяется
только к живому, копия пропускается; переиспользует фикстуру
`image_live_store_plus_blocked_copy` из Task 4):
...
```rust
    ...
    fn nvar_set_skips_blocked_copy_when_live_store_exists() {
        ...
        let blocked = flatten(&img.root.children[1].children[0].body)
            ...
            .unwrap();
        assert_eq!(
            img.root.children[1].children[0].body[blocked.data_off],
            0,
...
В `nvar.rs` (импорт ImageMode: `use crate::types::{FfsNode, FfsType, Guid, Image, ImageMode};`):
...
pub fn nvar_set(
    ...
    collect_var_hits(
        &image.root, &mut path, false, None, name, guid, offset, width, &mut found,
    )?;
...
- Modify: `crates/uefi-cli/tests/mock_server.rs`, `crates/uefi-tui/tests/mock_server.rs`, `crates/uefi-gateway/tests/mock_server.rs` (заглушки `nvar_list`/`nvar_set` — моки реализуют `EngineService` целиком, без них E0046 валит `cargo test --all`)
...
- [ ] **Step 4a: Заглушки в тестовых моках (cargo test --all)**

`EngineService` вручную реализован в трёх тестовых моках (cli/tui/gateway);
новые rpc-методы без заглушек дают E0046 (not all trait items implemented)
в `cargo test --all`. По образцу пустого `hii_form_tree` добавить в каждый
мок:
...
```rust
    ...
    fn seed_lookup_reads_varstore_record_bytes() {
        ...
        assert_eq!(seed_lookup(&img.root, "Setup", 114, 113, 2), None, "off+w > size");
...
Новые proto-поля ломают исчерпывающие struct-литералы `QuestionInfo`/
`QuestionSummary` вне engine (E0063: missing fields) — дополнить их
нейтральными `seed_value: None, seed_option: None` /
`seed_value: None, ifr_default: None` (литералы с `..Default::default()`
не трогать): `uefi-cli/src/output.rs`, `uefi-cli/tests/mock_server.rs`,
`uefi-tui/tests/mock_server.rs`, `uefi-tui/src/commands.rs` (точный список —
по выводу `cargo test --all`). Затем `cargo test --all`.

- [ ] **Step 6: Commit**

```bash
git add crates/uefi-proto/proto/engine.proto crates/uefi-engine/src/hii/mod.rs crates/uefi-cli crates/uefi-tui
...
(терминал 96 колонок: строка вопроса из `question_line` — 48 клеток
(глиф+пробел 2 + `{prompt:<28}` + ` q{:#x} {kind} ` 15 + `= 1` 3);
при 80 правая панель 60% даёт inner 46 — суффикс обрезается до `=`,
`contains("= 1")` не проходит; при 96 inner ≥ 48 суффикс виден целиком)
...
Существующие тесты forms.rs адаптировать под новый контракт (без этого
Step 4 не PASS):
- `form_panel_builds_zones`: `p.questions[N].contains(...)` →
  `p.questions[N].to_string().contains(...)` — у `Line` нет метода
  `contains`, но ratatui 0.28 даёт `Line: Display` (склейка span-контента);
- `question_bottom_branches_and_path_row`: `p.bottom.len() == 2` →
  `== 4` (заголовок + store + `Value (NVAR seed): —` + `IFR default: —`).
