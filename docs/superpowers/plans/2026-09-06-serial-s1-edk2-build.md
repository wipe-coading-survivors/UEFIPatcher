...
|---|---|
| `docker/edk2-builder.containerfile` | Образ сборки: runtime-base + gcc/make/python3/libuuid-devel/git/nasm + маркер ENV |
...
RUN dnf install -y git nasm gcc make python3 libuuid-devel && dnf clean all
ENV EDK2_BUILDER_CONTAINER=1
WORKDIR /work
```

Примечание (rule-11, обнаружено при выполнении Task 2 Step 4): без `git` в образе контейнерный режим `build_serial.sh` падает на `git -C /src/edk2 archive …` (`git: command not found`); git нужен именно внутри контейнера, т.к. archive-копия делается в container-mode ветке скрипта. Без `nasm` падает компиляция X64-ассемблера BaseLib (`nasm: command not found`, CpuId.nasm и др.) — тулчейн GCC X64 в edk2 требует nasm. Остальные host-инструменты тулчейна (gcc/gcc-ar/ld) входят в пакет gcc; `Trim` доступен через python-обёртку BinWrappers/PosixLike (в PATH после edksetup).
...
Run:
```bash
podman-remote run --rm uefipatcher-edk2-builder:latest bash -c 'git --version | head -1; nasm -v | head -1; gcc --version | head -1; make --version | head -1; python3 --version; echo EDK2_BUILDER_CONTAINER=$EDK2_BUILDER_CONTAINER'
```
Expected: строки версий git/nasm/gcc/make/python3 и `EDK2_BUILDER_CONTAINER=1`, exit 0.
...
Примечание к `mkdir -p …mipisyst…/brotli…` (rule-11, обнаружено при выполнении Step 4): MdePkg.dec и MdeModulePkg.dec декларируют [Includes]-пути внутрь подмодулей (mipisyst, brotli); парсер DEC валидирует существование путей при разборе, а в git-archive-копии подмодулей нет вовсе → `MdePkg.dec(33)`/`MdeModulePkg.dec(31): error 000E: … is not found`. Полный скан обоих .dec (все секции [Includes*]) показал ровно два отсутствующих пути — эти два; ни один из наших модулей эти заголовки не включает (DebugLib — Null, brotli-секций нет), поэтому пустых каталогов достаточно.
...
## 1. Стенд
- docker/edk2-builder.containerfile (fedora:44/runtime-base + gcc/make/python3/libuuid-devel/git/nasm)
