...
Задокументировать в §7 спеки сценарий владельческого прогона: открыть 450x write → Forms View → курсор на IntelRCSetup → `u` → статус «flips applied», пункт появляется в корневом меню (ветка a) / Chipsat-форма получает GOTO (ветка b). Не исполнять от имени владельца — оставить гейт за ним.
