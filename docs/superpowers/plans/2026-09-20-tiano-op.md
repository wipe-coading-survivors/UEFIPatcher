...
- Create: `crates/uefi-engine/tests/fixtures/tiano/c275-min.in`, `c275-min.expected`, `c275-median.in`, `c275-median.expected`, `c275-max.in`, `c275-max.expected`
  (позже гейт Task 4 добавил четвёртую пару `226d2-min.{in,expected}` — pbit=5, коммит `617e0f5`)
...
Expected: FAIL — `decompress_tiano_real_section` и `decompress_tiano_variant_fallback` падают с `Unsupported`.
...
*(Поправка 2026-09-20: fallback и фикстура 226d2-min (pbit=5, оракульная)
добавлены по находке живого гейта Task 4 — 226D2IL3.x несёт algo-1 секции
Tiano pbit=5; коммит `617e0f5`.)*
