...
mod tests {
    ...
    fn questions_unknown_form_empty_and_checkbox_width() {
        let pkg = package(&[form_set(), form(10031), checkbox(0x0300, 0x0060), end(), end(), end()]);
        assert!(questions(&pkg, 99999).is_empty());
...
```rust
    ...
    fn list_questions_bad_target_and_unknown_form() {
        ...
        let target = format!("{}:0x19:0", VENDOR_FORMSET_GUID_STR.to_lowercase());
        assert!(list_questions(&image, &target, 99999).unwrap().is_empty());
    }
```

Run: `cargo test -p uefi-engine list_questions`
Expected: `list_questions_reports_vendor_form` FAIL (пустышка возвращает NotFound), второй может PASS.

- [ ] **Step 3: prompt_texts в questions.rs**

В `crates/uefi-engine/src/hii/questions.rs` добавить (use-строки из Task 1 Step 1 уже наверху файла):
