...
```rust
    ...
    pub fn strings_cursor_down(&mut self) {
        let n = self.strings_visible().len();
        if n > 0 && self.forms.strings_cursor + 1 < n {
            self.forms.strings_cursor += 1;
        }
    }

    pub fn strings_cursor_up(&mut self) {
        if self.forms.strings_cursor > 0 {
            self.forms.strings_cursor -= 1;
        }
    }
```
...
```rust
    use crate::forms::{FormKey, FormsRow};
...
fn render_strings(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    if visible.is_empty() {
        ...
        state.select(Some(
            app.forms.strings_cursor.min(visible.len() - 1),
        ));
    }
