...
5. **Записи $SPF (72Б)**: скан движковой подписи (F8×8 @+36, tail 0100 0100
   ...
   (класс 0x0001 << 16) | глобальный индекс вопроса** (монотонный по позиции в
   IFR; максимум ~148 → новые индексы от 0x95).
...
7. **IFR-сдвиг**: вставка опкодов после q35 (конец вопросов формы 10019,
   pkg+0x993) сдвигает опкоды последующих форм на delta → записи $SPF с
   ifr >= точки вставки правим in-place (ifr += delta), инвариант-тест 389/389
   после пересборки. Формсеты других файлов (записи 2-го класса) не затронуты.
...
- Produces (все — чистые функции над `&mut Vec<u8>` контейнера $SPF,
  ...
  - `pub fn clone_page_with_controls(body: &mut Vec<u8>, page_offset: usize, extra_ctrls: &[u32]) -> usize` —
    ...
    `0x20 + 4*cnt (+4 хвостовой слот — сохранить по образцу: если исходная
    страница имела хвост, клон сохраняет хвост перед доп. контролами)),
  ...
  - `pub fn bump_container_length(body: &mut [u8], new_len: usize)` —
    правит поле длины контейнера в заголовке (последний header-офсет региона;
    найти по значению == текущая длина, tse-bridge §3: «последний header-офсет
    = длина контейнера»; тест фиксирует точное поле на LIVE-фрагменте).
...
- [ ] **Step 1: failing-тест** `real_image_ops_insert_serial_s3` (копия
  ...
  10019 → клон, cnt 6→8; (4) ifr-инвариант: после пересборки для всех 389+2
  записей ifr@+0x1C == фактическая позиция qid-опкода в forms-пакете
  (сверка сканом); (5) дифф образа confined: окно S2 [0xB63B18, 0xB81C18) +
