...
- Produces:
  ...
  - `pub fn add_question(image: &mut Image, item_id: &str, schema: &QuestionAddSchema) -> Result<AddQuestionResult, HiiError>`:
    ...
    их; 357 foreign-записей (2-й класс, видимы сканеру, резолвятся в ДРУГИХ
    форм-пакетах образа — доминирующе abbce13d, IntelRC-класс) не трогать:
    их ifr-координаты чужие, сплайс Setup-пакета их не сдвигает →
...
- [ ] **Step 1: failing-тест** `real_image_ops_insert_serial_s3` (копия
  ...
  F8-записей Setup-формсету принадлежат 32 — резолвятся (qid, ifr) в
  forms-пакете Setup; 357 foreign резолвятся в ДРУГИХ форм-пакетах образа
  (доминирующе abbce13d, IntelRC-класс) — их ifr-координаты чужие, сплайс
  Setup-пакета их не сдвигает, потому не тронуты; предикат резолва —
  `spf_record_resolves`):
