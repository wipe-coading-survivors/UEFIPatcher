...
7. **IFR-сдвиг**: вставка опкодов после q35 (конец вопросов формы 10019,
   ...
   из 389 F8-записей лишь 34 принадлежат Setup-формсету (резолвятся (qid, ifr)
   в forms-пакете Setup — совпадает с масс-проверкой tse-bridge §2 «34/34»);
   остальные 355 (2-й класс, прочие формсеты) ВИДИМЫ движковому сканеру
   (предположение пре-check «2-й класс без подписи» — неверно), их ifr
   не резолвится ни в одном пакете образа и семантика не декодирована →
   fixup избирательный: до IFR-мутации снять карту резолвящихся записей,
   после сплайса сдвинуть только их (ifr += delta при ifr >= точки вставки);
   foreign-записи не изменяются вовсе. Формсеты других файлов — не тронуты
   (восстановленный замысел этого пункта).
...
- Produces:
  ...
  - `pub fn add_question(image: &mut Image, item_id: &str, schema: &QuestionAddSchema) -> Result<AddQuestionResult, HiiError>`:
    ...
    Setup-резолвящихся записей (34 на LIVE), после сплайса сдвинуть только
...
- [ ] **Step 1: failing-тест** `real_image_ops_insert_serial_s3` (копия
  ...
  (a) R_base = резолвящиеся записи базового образа (live = 34, вычислять,
  ...
  fixup); (5) дифф образа confined: окно S2 [0xB63B18, 0xB81C18) +
